...
{
    // format,                                   type,                              internal_format,                           swizzle_r swizzle_g swizzle_b swizzle_a
...
GLuint vglLoadTexture(const char* file_name, image_t* image)
{
    ...
    switch (image->target)
    {
        ...
        case GL_TEXTURE_1D:
            glTexStorage1D(image->target, image->levels, image->internal_format, image->mips[0].width);
            for (int level = 0; level < image->levels; ++level)
                glTexSubImage1D(GL_TEXTURE_1D, level, 0, image->mips[level].width, image->format, image->type, image->mips[level].data);
            break;

        case GL_TEXTURE_1D_ARRAY:
            glTexStorage2D(image->target, image->levels, image->internal_format, image->mips[0].width, image->slices);
            for (int level = 0; level < image->levels; ++level)
                glTexSubImage2D(GL_TEXTURE_1D, level, 0, 0, image->mips[level].width, image->slices, image->format, image->type, image->mips[level].data);
            break;

        case GL_TEXTURE_2D:
            glTexStorage2D(image->target, image->levels, image->internal_format, image->mips[0].width, image->mips[0].height);
            for (int level = 0; level < image->levels; ++level)
                glTexSubImage2D(GL_TEXTURE_2D, level, 0, 0, image->mips[level].width, image->mips[level].height, image->format, image->type, image->mips[level].data);
            break;
        ...
        case GL_TEXTURE_2D_ARRAY:
            glTexStorage3D(image->target, image->levels, image->internal_format, image->mips[0].width, image->mips[0].height, image->slices);
            for (int level = 0; level < image->levels; ++level)
                glTexSubImage3D(GL_TEXTURE_2D_ARRAY, level, 0, 0, 0, image->mips[level].width, image->mips[level].height, image->slices, image->format, image->type, image->mips[level].data);
            break;
        ...
        case GL_TEXTURE_3D:
            glTexStorage3D(image->target, image->levels, image->internal_format, image->mips[0].width, image->mips[0].height, image->mips[0].depth);
            for (int level = 0; level < image->levels; ++level)
            {
...
static bool vgl_DDSHeaderToImageDataHeader(const DDS_FILE_HEADER& header, image_t* image)
{
    if (header.std_header.ddspf.dwFlags == DDS_DDPF_FOURCC && header.std_header.ddspf.dwFourCC == DDS_FOURCC_DX10)
    {
        if (header.dxt10_header.format < NUM_DDS_FORMATS)
        {
            ...
            image->internal_format = format.internal_format;
    ...
    image->format = image->type = image->internal_format = GL_NONE;
...
static GLsizei vgl_GetDDSStride(const DDS_FILE_HEADER& header, GLsizei width)
{
    ...
    else
    {
        switch (header.std_header.ddspf.dwFlags)
        {
            case DDS_DDPF_RGB: return width * 3;
            case (DDS_DDPF_RGB | DDS_DDPF_ALPHA):
...
void vglLoadDDS(const char* filename, image_t* image)
{
    ...
    image->mips[0].data = (uint8_t*) malloc (image->size * sizeof(uint8_t));
    ...
    if (image->levels == 0) image->levels = 1;

    for (level = 0; level < image->levels; ++level)
    {
        image->mips[level].data = ptr;
