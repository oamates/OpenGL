//========================================================================================================================================================================================================================
// Windows specific thread-related (safe) functionality
// Platform : WinAPI
...
MutexImpl::MutexImpl(bool recursive)
{
    ...
    // Older versions of Windows don't support CreateSemaphoreEx, so stick with CreateSemaphore for portability.
...
{
    if (::WaitForSingleObject(hMutexOrSemaphore, 0) != WAIT_OBJECT_0)
        return false;
    LockCount++;
    return true;
}
...
void Event::updateState(bool newState, bool newTemp, bool mustNotify)
{
    ...
    // NOTE: The lock does not need to be held when calling notify_all(), and holding it is in fact a pessimization.
    if (mustNotify)
