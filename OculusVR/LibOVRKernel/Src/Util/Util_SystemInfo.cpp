...
const char* OSAsString()
{
    ...
    return Is64BitWindows() ? "Win64" : "Win32";
...
String GetFileVersionStringW(const wchar_t filePath[MAX_PATH])
{
    ...
    {
        auto pVersionInfo = std::unique_ptr<BYTE[]>(new BYTE (dwSize));
        if (!pVersionInfo)
        {
...
void GetGraphicsCardList(OVR::Array<OVR::String>& gpus) 
{
    ...
    if (file)
    {
        ...
        while (status >= 0) {
            status = fscanf(file, "%*[^\n]\n"); // Read up till the end of the current line, leaving the
            // file pointer at the beginning of the next line skipping any leading whitespace.
            OVR_UNUSED(status);
...
std::string GetProcessPath(pid_t processId, bool fileNameOnly, bool enableErrorResults)
{
    ...
    ScopedProcessHANDLE processHandle(OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, processId));
...
std::string GetProcessPath(HANDLE processHandle, bool fileNameOnly, bool enableErrorResults)
{
    ...
    if (QueryFullProcessImageNameW(processHandle, 0, procPathW, &processNameLength) != 0)
    {
        ...
        if (fileNameOnly)
        {
            ...
            if (_splitpath_s(result.c_str(), nullptr, 0, nullptr, 0, fileName, _MAX_FNAME, fileExtension, _MAX_EXT) == 0)
            {
                result = fileName;
    ...
    else if (enableErrorResults)
    {
        ...
        char buffer[256];
        snprintf(buffer, sizeof(buffer), "(QueryFullProcessImageNameW failure for process handle %llx, pid %u. Error code: %u)", (uint64_t)processHandle, dwProcessId, dwLastError);
        result = buffer;
    }
...
bool GetRegistryStringW(const wchar_t* pSubKey, const wchar_t* stringName, wchar_t out[MAX_PATH], bool wow64value, bool currentUser)
{
    ...
    if ((RegOpenKeyExW(root, pSubKey, 0, KEY_QUERY_VALUE | (wow64value ? KEY_WOW64_32KEY : KEY_WOW64_64KEY), &hKey) != ERROR_SUCCESS) ||
        (RegQueryValueExW(hKey, stringName, NULL, &dwType, (LPBYTE)&value, &value_length) != ERROR_SUCCESS) ||
        (dwType != REG_SZ))
    {
        out[0] = L'\0';
        RegCloseKey(hKey);
...
bool GetRegistryDwordW(const wchar_t* pSubKey, const wchar_t* stringName, DWORD& out, bool wow64value, bool currentUser)
{
    ...
    if ((RegOpenKeyExW(root, pSubKey, 0, KEY_QUERY_VALUE | (wow64value ? KEY_WOW64_32KEY : KEY_WOW64_64KEY), &hKey) != ERROR_SUCCESS) ||
        (RegQueryValueExW(hKey, stringName, NULL, &dwType, (LPBYTE)&out, &value_length) != ERROR_SUCCESS) ||
        (dwType != REG_DWORD))
    {
        out = 0;
        RegCloseKey(hKey);
...
String GetMachineTags()
{
    ...
    bool result = OVR::Util::GetRegistryStringW(OculusRegKey, MachineTagRegValueName, stringValue, false, false);
    if (result) {
        allTags = stringValue;
    }

    // HKEY_CURRENT_USER\Software\Oculus\MachineTags SZ <tags>
    result = OVR::Util::GetRegistryStringW(OculusRegKey, MachineTagRegValueName, stringValue, false, true);
    if (result)
    {
        if (!allTags.empty())
        {
...
ProcessMemoryInfo GetCurrentProcessMemoryInfo()
{
    ...
    if (::GetProcessMemoryInfo(GetCurrentProcess(), reinterpret_cast<PROCESS_MEMORY_COUNTERS*>(&pmce), sizeof(pmce)))
    {
        ...
        pmi.UsedMemory = (uint64_t)pmce.WorkingSetSize; 
    }
  #else
...
class PdhHelper : public SystemSingletonBase<PdhHelper>
{
  ...
  // RateCounter encapsulates a single counter that represents rate information.
    struct RateCounter
    {
        ...
        // PdhAddCounterW, so be mindful if adding a bunch of counter objects in synchronous or blocking manner.
        void Init(PDH_HQUERY hQuery, LPCWSTR counterPath)
        {
            if (!hQuery) return;
            PDH_STATUS status = PdhAddCounterW(hQuery, counterPath, 0, &CounterHandle);
            if (ERROR_SUCCESS != status) return;
            Valid = true;
        }
        ...
        double Query()
        {
            ...
            if (ERROR_SUCCESS == status && PDH_CSTATUS_VALID_DATA == now.CStatus)
            {
                ...
                double result = (HasLast && (now.SecondValue - Last.SecondValue)) ? 
                    double(now.FirstValue - Last.FirstValue) / (double(now.SecondValue - Last.SecondValue) * Timer::GetPerfFrequencyInverse()) : 0.0;
                Last = now;
                HasLast = true;
                return result;
            }
...
PdhHelper::PdhHelper()
{
    ...
    {
        // If this fails, PdhQuery will be null, and subsequent query will check for this and return default data if we have no query object.
        CleanUp();
    }
...
SystemMemoryInfo GetSystemMemoryInfo()
{
    ...
    if (ph)
    {
        // A single Collect call is required for the query object regardless of how many counters we are interested in.
        if (ph->Collect())
        {
...
CPUInstructionSet GetSupportedCPUInstructionSet(bool* popcntSupported, bool* lzcntSupported)
{
    ...
    if ((features[3] & (1 << 26)) == 0) return cpuIS;   // If SSE2 is not supported...
    cpuIS = CPUInstructionSet::SSE2;

    if ((features[2] & (1 << 0)) == 0) return cpuIS;    // If SSE3 is not supported...
    cpuIS = CPUInstructionSet::SSE3;

    if ((features[2] & (1 << 9)) == 0) return cpuIS;    // If SSSE3 is not supported...
    cpuIS = CPUInstructionSet::SSSE3;

    if ((features[2] & (1 << 19)) == 0) return cpuIS;   // If SSE4.1 is not supported...
    cpuIS = CPUInstructionSet::SSE41;

    if ((features[2] & (1 << 20)) == 0) return cpuIS;   // If SSE42 is not supproted...
    ...
    cpuid(features, 7);
    if ((features[1] & (1 << 5)) == 0) return cpuIS;    // If AVX2 is not supported...
