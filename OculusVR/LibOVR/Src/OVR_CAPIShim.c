...
// Unless required by applicable law or agreed to in writing, the Oculus VR SDK distributed under the License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS 
...
// If defined then our shared library loading code searches for developer build directories.
//========================================================================================================================================================================================================================
...
// If you want to control the behavior of these flags, then explicitly define either -DOVR_BUILD_RELEASE or -DOVR_BUILD_DEBUG in the compiler arguments.
//========================================================================================================================================================================================================================
...
static ovrBool OVR_isBundleFolder(const char* filePath)
{
    ...
    for (size_t i = 0; i < sizeof(extensionArray) / sizeof(extensionArray[0]); i++)
    {
...
#if defined(OVR_ENABLE_DEVELOPER_SEARCH)
// Returns true if the path begins with the given prefix. Doesn't support non-ASCII paths, else the return value may be incorrect.
...
//========================================================================================================================================================================================================================
// The appContainer argument is specific currently to only Macintosh. If true and the application is a .app bundle then it returns the location of the bundle and not the path 
// to the executable within the bundle. Else return the path to the executable binary itself. The moduleHandle refers to the relevant dynamic (a.k.a. shared) library. The main 
// executable is the main module, and each of the shared libraries is a module. This way you can specify that you want to know the directory of the given shared library, which may 
// be different from the main executable. If the moduleHandle is NULL then the current application module is used.
//========================================================================================================================================================================================================================
    ...
    if (result == 0)                                                    // If success...
    {
        ...
        if (realpath(directoryPath, realPath))                          // realpath returns the canonicalized absolute file path.
        {
            ...
            if (appContainer)                                           // If the caller wants the path to the containing bundle...
            {
                ...
                while (!pathIsContainer && strncmp(containerPath, ".", OVR_MAX_PATH) && strncmp(containerPath, "/", OVR_MAX_PATH)) 
                {                                                       // While the container we're looking for is not found and while the path doesn't start with a . or /
...
//========================================================================================================================================================================================================================
// Get the file path to the current module's (DLL or EXE) directory within the current process. Will be different from the process module handle if the current module is a DLL and is in a
// different directory than the EXE module. If successful then directoryPath will be valid and ovrTrue is returned, else directoryPath will be empty and ovrFalse is returned.
//========================================================================================================================================================================================================================
...
typedef WINCRYPT32API DWORD(WINAPI* PtrCertGetNameStringW)(PCCERT_CONTEXT pCertContext, DWORD dwType, DWORD dwFlags, void* pvTypePara, LPWSTR pszNameString, DWORD cchNameString);
...
typedef CRYPT_PROVIDER_SGNR*(WINAPI* PtrWTHelperGetProvSignerFromChain)(CRYPT_PROVIDER_DATA* pProvData, DWORD idxSigner, BOOL fCounterSigner, DWORD idxCounterSigner);
...
static ValidateCertificateContentsResult ValidateCertificateContents(CertificateEntry* chain, CRYPT_PROVIDER_SGNR* cps)
{
    ...
    for (certIndex = 0; certIndex < ExpectedNumCertificates; ++certIndex)
    {
        ...
        m_PtrCertGetNameStringW(pCertData->pCert, CERT_NAME_ATTR_TYPE, 0, szOID_COMMON_NAME, subjectStr, ARRAYSIZE(subjectStr));
        m_PtrCertGetNameStringW(pCertData->pCert, CERT_NAME_ATTR_TYPE, CERT_NAME_ISSUER_FLAG, 0, issuerStr, ARRAYSIZE(issuerStr));

        if (wcscmp(subjectStr, chain[certIndex].Subject) != 0 || wcscmp(issuerStr, chain[certIndex].Issuer) != 0) 
...
#define OVR_SIGNING_CONVERT_PTR(ftype, fptr, procaddr) { union { ftype p1; ModuleFunctionType p2; } u; u.p2 = procaddr; fptr = u.p1; }
...
static HANDLE OVR_Win32_SignCheck(FilePathCharType* fullPath)
{
    ...
    if (m_PtrCertGetNameStringW == NULL || m_PtrWinVerifyTrust == NULL || m_PtrWTHelperProvDataFromStateData == NULL || m_PtrWTHelperGetProvSignerFromChain == NULL)
        return INVALID_HANDLE_VALUE;

    if (!fullPath)
        return INVALID_HANDLE_VALUE;

    hFile = CreateFileW(fullPath, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_READONLY, 0);
    ...
    resultStatus = m_PtrWinVerifyTrust((HWND)INVALID_HANDLE_VALUE, &actionGUID, &wintrustData);
    if (resultStatus == ERROR_SUCCESS && wintrustData.hWVTStateData != 0 && wintrustData.hWVTStateData != INVALID_HANDLE_VALUE)
    {
    ...
    m_PtrWinVerifyTrust((HWND)INVALID_HANDLE_VALUE, &actionGUID, &wintrustData);
...
#else
    *result = ovrSuccess;
    if (access(libraryPath, X_OK | R_OK) != 0)                                  // Don't bother trying to dlopen() a file that is not even there.
    {
        *result = ovrError_LibPath;
        return NULL;
    }
    ...
    // Use RTLD_NOW because we don't want unexpected stalls at runtime, and thelibr ary isn't very large.
...
{
#if defined(_WIN32)
        // We may need to consider what to do in the case that the library is in an exception state. In a Windows C++ DLL, all global objects (including static members of classes) will be
        // constructed just before the calling of the DllMain with DLL_PROCESS_ATTACH and they will be destroyed just after the call of the DllMain with DLL_PROCESS_DETACH. 
        // We may need to intercept DLL_PROCESS_DETACH and have special handling for the case that the DLL is broken.
    if (hLibrary)
...
//========================================================================================================================================================================================================================
static ModuleHandleType OVR_FindLibraryPath(int requestedProductVersion, int requestedMajorVersion, FilePathCharType* libraryPath, size_t libraryPathCapacity, ovrResult* result)
{
    ...
    *result = ovrError_LibLoad;
    ...
    // Note: OVR_ENABLE_DEVELOPER_SEARCH is deprecated in favor of the simpler LIBOVR_DLL_DIR, as the edge case uses of the former created some complications that 
    // may be best solved by simply using a LIBOVR_DLL_DIR environment variable which the user can set in their debugger or system environment variables.
...

    {
        ...
        if (pLibOvrDllDir)
        {
            char developerDir8[OVR_MAX_PATH];                                   // If missing a trailing path separator then append one.
...
// Support checking for a developer library location override via the OVR_SDK_ROOT environment variable. This pathway is deprecated in favor of using LIBOVR_DLL_DIR instead.
#if defined(OVR_ENABLE_DEVELOPER_SEARCH)
    if (!developerDir[0])                                                       // If not already set by LIBOVR_DLL_PATH...
    {
    
        char sdkRoot[OVR_MAX_PATH];                                             // __FILE__ maps to <sdkRoot>/LibOVR/Src/OVR_CAPIShim.c
        char* pLibOVR;
        size_t i;

        // We assume that __FILE__ returns a full path, which isn't the case for some compilers.
        // Need to compile with /FC under VC++ for __FILE__ to expand to the full file path. clang expands __FILE__ to a full path by default.
        ...
        if (sdkRoot[0])
        {
            // We want to use a developer version of the library only if the application is also being executed from a developer location. Ideally we would do this by checking 
            // that the relative path from the executable to the shared library is the same at runtime as it was when the executable was first built, but we don't have an easy 
            // way to do that from here and it would require some runtime help from the application code. Instead we verify that the application is simply in the same developer 
            // tree that was was when built. We could put in some additional logic to make it very likely to know if the EXE is in its original location.
            FilePathCharType modulePath[OVR_MAX_PATH];
            const ovrBool pathMatch = OVR_GetCurrentModuleDirectory(modulePath, OVR_MAX_PATH, ovrTrue) && (OVR_PathStartsWith(modulePath, sdkRoot) == ovrTrue);
...
#undef CONFIG_VARIANT
...
#if defined(_WIN32)
            int count = swprintf_s(developerDir, OVR_MAX_PATH, L"%hs\\LibOVR\\Lib\\Windows\\%hs\\%hs\\%hs\\", sdkRoot, pArchDirName, pConfigDirName, pCompilerVersion);
#elif defined(__APPLE__)                                                        // Apple/XCode doesn't let you specify an arch in build paths, which is OK if we build a universal binary.
            (void) pArchDirName;
            int count = snprintf(developerDir, OVR_MAX_PATH, "%s/LibOVR/Lib/Mac/%s/", sdkRoot, pConfigDirName);
#else
            int count = snprintf(developerDir, OVR_MAX_PATH, "%s/LibOVR/Lib/Linux/%s/%s/", sdkRoot, pArchDirName, pConfigDirName);
#endif
...

    {
        ...
        size_t i;
#if defined(_WIN32)
        const FilePathCharType* directoryArray[2];                              // On Windows, only search the developer directory and the usual path
...
#elif defined(__APPLE__)                                                        // https://developer.apple.com/library/mac/documentation/Darwin/Reference/ManPages/man1/dyld.1.html
        ...
        else                                                                    // https://developer.apple.com/library/mac/documentation/Darwin/Reference/ManPages/man3/getpwuid_r.3.html
        {
    ...
    #define STR(x) STR1(x)
        ...
        // Since we are manually loading the LibOVR dynamic library, we need to look in various locations for a file that matches our requirements. The functionality required 
        // is somewhat similar to the operating system's dynamic loader functionality. Each OS has some differences in how this is handled. Future versions of this may iterate 
        // over all libOVRRT.so.* files in the directory and use the one that matches our requirements.
        // We need to look for a library that matches the product version and major version of the caller's request, and that library needs to support a minor version that 
        // is >= the requested minor version. Currently we don't test the minor version here, as the library is named based only on the product and major version. Currently the 
        // minor version test is handled via the initialization of the library and the initialization fails if minor version cannot be supported by the library. The reason this 
        // is done during initialization is that the library can at runtime support multiple minor versions based on the user's request. To the external user, all that matters 
        // is that they call ovr_Initialize with a requested version and it succeeds or fails.
        // The product version is something that is at a higher level than the major version, and is not something that's always seen in libraries (an example is the well-known 
        // LibXml2 library, in which the 2 is essentially the product version).
...
#else
            // Unix. Applications that depend on the OS (e.g. ld-linux / ldd) can rely on the library being in a common location such as /usr/lib or can rely on the -rpath linker 
            // option to embed a path for the OS to check for the library, or can rely on the LD_LIBRARY_PATH environment variable being set. It's generally not recommended that 
            // applications depend on LD_LIBRARY_PATH be globally modified, partly due to potentialy security issues. Currently we check the current application directory, current 
            // working directory, and then /usr/lib and possibly others.
...
//========================================================================================================================================================================================================================
// ***** hLibOVR - global handle to the LivOVR shared library.
//========================================================================================================================================================================================================================
...
//========================================================================================================================================================================================================================
#define OVR_DECLARE_IMPORT(ReturnValue, FunctionName, OptionalVersion, Arguments) union { ReturnValue(OVR_CDECL* Ptr) Arguments; ModuleFunctionType Symbol; } FunctionName;
...
static ovrResult OVR_LoadSharedLibrary(int requestedProductVersion, int requestedMajorVersion)
{
    ...
    memset(&API, 0, sizeof(API));                                               // Zero the API table just to be paranoid
    ...
    OVR_LIST_APIS(OVR_GETFUNCTION, OVR_IGNORE_IMPORT)                           // Load the current API entrypoint using the catenated FunctionName and OptionalVersion
  ...
  FailedToLoadSymbol:
    OVR_UnloadSharedLibrary();                                                  // Check SymbolName for the name of the API which failed to load
    return result;
}

static const ovrInitParams DefaultParams =                                      // These defaults are also in CAPI.cpp
{
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_Initialize(const ovrInitParams* inputParams) 
{
    ...
    typedef void(OVR_CDECL * ovr_ReportClientInfoType)(unsigned int compilerVersion, int productVersion, int majorVersion, int minorVersion, int patchVersion, int buildNumber);
    ovr_ReportClientInfoType reportClientInfo;

    uint8_t crc = 0;                                                            // Do something with our version signature hash to prevent it from being optimized out, e.g. compute a cheap CRC.
    size_t i;

    for (i = 0; i < (sizeof(OculusSDKUniqueIdentifier) - 3); ++i)               // Minus 3 because we have trailing OVR_MAJOR_VERSION, OVR_MINOR_VERSION,
        crc ^= OculusSDKUniqueIdentifier[i];                                    // OVR_PATCH_VERSION which vary per version.
    ...
    {
        params = *inputParams;
        if (!(params.Flags & ovrInit_RequestVersion))                           // If not requesting a particular minor version, enable requesting the default minor version.
        {
            params.Flags |= ovrInit_RequestVersion;
            params.RequestedMinorVersion = OVR_MINOR_VERSION;
        }
    }
    params.Flags &= ovrinit_WritableBits;                                       // Clear non-writable bits provided by client code.

    // Error out if the requested minor version is less than our lowest deemed compatible version denoted by OVR_MIN_REQUESTABLE_MINOR_VERSION.
    // This code has to be in the shim as we want to enforce usage of the new API versions for applications being recompiled while maintaining backwards compatibility with older apps
    if (params.RequestedMinorVersion < OVR_MIN_REQUESTABLE_MINOR_VERSION)       // Requested LibOVRRT version too low
    {
    ...
    result = OVR_LoadSharedLibrary(OVR_PRODUCT_VERSION, OVR_MAJOR_VERSION);     // By design we ignore the build version in the library search.
...
#endif // _MSC_FULL_VER
        reportClientInfo(mscFullVer, OVR_PRODUCT_VERSION, OVR_MAJOR_VERSION, OVR_MINOR_VERSION, OVR_PATCH_VERSION, OVR_BUILD_NUMBER);
    }
    return result;
}
...
{
    // We don't directly return the value of the DLL API.ovr_GetVersionString.Ptr call, because that call returns a pointer to memory within the DLL. If the DLL goes
    // away then that pointer becomes invalid while the process may still be holding onto it. So we save a local copy of it which is always valid.
...
#if defined(_WIN32)
OVR_PUBLIC_FUNCTION(ovrResult) ovr_CreateTextureSwapChainDX(ovrSession session, IUnknown* d3dPtr, const ovrTextureSwapChainDesc* desc, ovrTextureSwapChain* outTextureSet)
{
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_CreateMirrorTextureDX(ovrSession session, IUnknown* d3dPtr, const ovrMirrorTextureDesc* desc, ovrMirrorTexture* outMirrorTexture)
{
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_CreateMirrorTextureWithOptionsDX(ovrSession session, IUnknown* d3dPtr, const ovrMirrorTextureDesc* desc, ovrMirrorTexture* outMirrorTexture)
{
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_GetTextureSwapChainBufferDX(ovrSession session, ovrTextureSwapChain chain, int index, IID iid, void** ppObject) 
{
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_GetMirrorTextureBufferDX(ovrSession session, ovrMirrorTexture mirror, IID iid, void** ppObject)
{
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_CreateTextureSwapChainGL(ovrSession session, const ovrTextureSwapChainDesc* desc, ovrTextureSwapChain* outTextureSet)
{
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_CreateMirrorTextureGL(ovrSession session, const ovrMirrorTextureDesc* desc, ovrMirrorTexture* outMirrorTexture)
{
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_CreateMirrorTextureWithOptionsGL(ovrSession session, const ovrMirrorTextureDesc* desc, ovrMirrorTexture* outMirrorTexture)
{
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_GetTextureSwapChainBufferGL(ovrSession session, ovrTextureSwapChain chain, int index, unsigned int* texId)
{
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_GetMirrorTextureBufferGL(ovrSession session, ovrMirrorTexture mirror, unsigned int* texId)
{
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_GetTextureSwapChainLength(ovrSession session, ovrTextureSwapChain chain, int* length)
{
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_GetTextureSwapChainCurrentIndex(ovrSession session, ovrTextureSwapChain chain, int* currentIndex)
{
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_GetTextureSwapChainDesc(ovrSession session, ovrTextureSwapChain chain, ovrTextureSwapChainDesc* desc)
{
...
{
    if (!API.ovr_DestroyTextureSwapChain.Ptr) return;
...
{
    if (!API.ovr_DestroyMirrorTexture.Ptr) return;
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_EndFrame(ovrSession session, long long frameIndex, const ovrViewScaleDesc* viewScaleDesc, ovrLayerHeader const* const* layerPtrList, unsigned int layerCount)
{
...
{
    if (!API.ovr_GetPredictedDisplayTime.Ptr) return 0.0;
    return API.ovr_GetPredictedDisplayTime.Ptr(session, frameIndex);
}

OVR_PUBLIC_FUNCTION(double) ovr_GetTimeInSeconds()
{
    if (!API.ovr_GetTimeInSeconds.Ptr) return 0.0;
...
{
    if (!API.ovr_GetBool.Ptr) return ovrFalse;
...
{
    if (!API.ovr_SetBool.Ptr) return ovrFalse;
...
{
    if (!API.ovr_GetInt.Ptr) return 0;
...
{
    if (!API.ovr_SetInt.Ptr) return ovrFalse;
...
{
    if (!API.ovr_GetFloat.Ptr) return 0.0f;
...
{
    if (!API.ovr_SetFloat.Ptr) return ovrFalse;
    return API.ovr_SetFloat.Ptr(session, propertyName, value);
}

OVR_PUBLIC_FUNCTION(unsigned int) ovr_GetFloatArray(ovrSession session, const char* propertyName, float values[], unsigned int arraySize)
{
    if (!API.ovr_GetFloatArray.Ptr) return 0;
    return API.ovr_GetFloatArray.Ptr(session, propertyName, values, arraySize);
}

OVR_PUBLIC_FUNCTION(ovrBool) ovr_SetFloatArray(ovrSession session, const char* propertyName, const float values[], unsigned int arraySize)
{
    if (!API.ovr_SetFloatArray.Ptr) return ovrFalse;
...
{
    if (!API.ovr_SetString.Ptr) return ovrFalse;
...
{
    if (!API.ovr_TraceMessage.Ptr) return -1;
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_GetExternalCameras(ovrSession session, ovrExternalCamera* outCameras, unsigned int* outCameraCount)
{
