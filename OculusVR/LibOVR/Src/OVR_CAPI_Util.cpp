...
OVR_PUBLIC_FUNCTION(ovrMatrix4f) ovrMatrix4f_OrthoSubProjection(ovrMatrix4f projection, ovrVector2f orthoScale, float orthoDistance, float hmdToEyeOffsetX)
{
    ...
    // We want to find the projection that maps the range [-FovPixels/2, FovPixels/2] to the physical [-orthoHalfFov, orthoHalfFov]
    // Note moving the offset from M[0][2] + M[1][2] to M[0][3] + M[1][3] - this means we don't have to feed in Z = 1 all the time.
    // The horizontal offset math is a little hinky because the destination is actually [-orthoHalfFov + orthoHorizontalOffset, orthoHalfFov + orthoHorizontalOffset]
    // So we need to first map [-FovPixels / 2, FovPixels / 2] to [-orthoHalfFov + orthoHorizontalOffset, orthoHalfFov + orthoHorizontalOffset]:
    // x1 = x0 * orthoHalfFov / (FovPixels / 2) + orthoHorizontalOffset = x0 * 2 * orthoHalfFov / FovPixels + orthoHorizontalOffset;
    // But then we need the sam mapping as the existing projection matrix, i.e.
    // x2 = x1 * Projection.M[0][0] + Projection.M[0][2] = x0 * (2*orthoHalfFov/FovPixels + orthoHorizontalOffset) * Projection.M[0][0] + Projection.M[0][2] = 
    //    = x0 * Projection.M[0][0] * 2 * orthoHalfFov / FovPixels + orthoHorizontalOffset * Projection.M[0][0] + Projection.M[0][2];
    // So in the new projection matrix we need to scale by Projection.M[0][0] * 2 * orthoHalfFov/FovPixels and offset by orthoHorizontalOffset * Projection.M[0][0] + Projection.M[0][2].
    //====================================================================================================================================================================================================================
    ...
    ortho.M[2][2] = 0.0f;
    ortho.M[2][3] = 0.0f;
    ortho.M[3][0] = 0.0f;                                                       // No perspective correction for ortho.
    ortho.M[3][1] = 0.0f;
    ortho.M[3][2] = 0.0f;
    ortho.M[3][3] = 1.0f;
...
{
    if (!hmdToEyeOffset || !outEyePoses) return;
    using OVR::Posef;
    using OVR::Vector3f;
...
{
    if (!hmdToEyePose || !outEyePoses) return;
    using OVR::Posef;
    using OVR::Vector3f;
...
{
    ovrDetectResult result;                                                                 // Initially we assume everything is not running.
    result.IsOculusHMDConnected = ovrFalse;
    result.IsOculusServiceRunning = ovrFalse;
#if defined(_WIN32)
    HANDLE hServiceEvent = ::OpenEventW(SYNCHRONIZE, FALSE, OVR_HMD_CONNECTED_EVENT_NAME);  // Attempt to open the named event.
    if (hServiceEvent != nullptr)                                                           // If event exists,
    {        
        result.IsOculusServiceRunning = ovrTrue;                                            // This indicates that the Oculus Runtime is installed and running.
        DWORD objectResult = ::WaitForSingleObject(hServiceEvent, timeoutMilliseconds);     // Poll for event state.
        if (objectResult == WAIT_OBJECT_0)                                                  // If the event is signaled,
            result.IsOculusHMDConnected = ovrTrue;                                          // This indicates that the Oculus HMD is connected.
...
static float wavPcmBytesToFloat(const void* data, int32_t sizeInBits, bool swapBytes) 
{
    ...
    // There's not a strong standard to convert 8/16/32b PCM to float. For 16b: MSDN says range is [-32760, 32760], Pyton Scipy uses [-32767, 32767] and Audacity
    // outputs the full range [-32768, 32767]. We use the same range on both sides and clamp to [-1, 1].

    float result = 0.0f;
    if (sizeInBits == 8)                                                                    // uint8_t is a special case, unsigned where 128 is zero
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_GenHapticsFromAudioData(ovrHapticsClip* outHapticsClip, const ovrAudioChannelData* audioChannel, ovrHapticsGenMode genMode)
{
    ...
    for (int32_t i = 0; i < hapticsSampleCount; ++i)
    {
        ...
        uint8_t hapticSample = (uint8_t)std::min(UCHAR_MAX, (int)round(fabs(sample) * kHapticsMaxAmplitude));
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_ReadWavFromBuffer(ovrAudioChannelData* outAudioChannel, const void* inputData, int dataSizeInBytes, int stereoChannelToUse)
{
    ...
    enum WavFormats                                                                         // We don't support any format other than PCM and IEEE Float
    {
    ...
    bool isValidWav = (wavId[0] == 'R' && wavId[1] == 'I' && wavId[2] == 'F' && wavId[3] == 'F') && memcmp(header->WavId, "WAVE", 4) == 0;
    bool hasValidChunks = memcmp(header->FmtChunckId, "fmt ", 4) == 0 && memcmp(header->DataChunckId, "data ", 4) == 0;
    ...
    bool isSupported = (header->Format == kWavFormatLPCM || header->Format == kWavFormatFloatIEEE) &&
                       (header->Channels == 1 || header->Channels == 2) && (header->BitsPerSample == 8 || header->BitsPerSample == 16 || header->BitsPerSample == 32);
    if (!isSupported)
    ...
    for (int32_t i = 0; i < blockCount; i++)
    {
        ...
        float sample = (header->Format == kWavFormatLPCM) ? wavPcmBytesToFloat(dataPtr, header->BitsPerSample, false) : *(float*) dataPtr;
        samples[i] = sample;
    }
