...
// Copyright 2015-2016 Oculus VR, LLC All Rights reserved.
//========================================================================================================================================================================================================================
...
//========================================================================================================================================================================================================================
// Enumerates modifications to the projection matrix based on the application's needs, see ovrMatrix4f_Projection
//========================================================================================================================================================================================================================
...
//========================================================================================================================================================================================================================
// Return values for ovr_Detect, see ovr_Detect
//========================================================================================================================================================================================================================
...
{
  ovrHapticsGenMode_PointSample,                                                // Point sample original signal at Haptics frequency
...
{
    const float* Samples;                                                       // Samples stored as floats [-1.0f, 1.0f].
...
// Checks for Oculus Runtime and Oculus HMD device status without loading the LibOVRRT shared library. 
// This may be called before ovr_Initialize() to help decide whether or not to initialize LibOVR.
//  -timeoutMilliseconds Specifies a timeout to wait for HMD to be attached or 0 to poll.
...
#ifdef _WIN32
    #define OVR_HMD_CONNECTED_EVENT_NAME L"OculusHMDConnected"                  /* This is the Windows Named Event name that is used to check for HMD connected state. */
#endif // _WIN32

//========================================================================================================================================================================================================================
// Used to generate projection from ovrEyeDesc::Fov.
//  -fov Specifies the ovrFovPort to use.
//  -znear Distance to near Z limit.
//  -zfar Distance to far Z limit.
//  -projectionModFlags A combination of the ovrProjectionModifier flags.
...
//  -projection Specifies the project matrix from which to extract ovrTimewarpProjectionDesc.
//  -projectionModFlags A combination of the ovrProjectionModifier flags.
...
//  -HmdToEyeOffsetX Specifies the offset of the eye from the center.
//  -return Returns the calculated projection matrix.
//========================================================================================================================================================================================================================
OVR_PUBLIC_FUNCTION(ovrMatrix4f) ovrMatrix4f_OrthoSubProjection(ovrMatrix4f projection, ovrVector2f orthoScale, float orthoDistance, float HmdToEyeOffsetX);

//========================================================================================================================================================================================================================
// Computes offset eye poses based on headPose returned by ovrTrackingState.
//  -headPose Indicates the HMD position and orientation to use for the calculation.
//  -hmdToEyePose Can be ovrEyeRenderDesc.HmdToEyePose returned from ovr_GetRenderDesc. For monoscopic rendering, use a position vector that is average of the two position vectors for each eyes.
//  -outEyePoses[out] If outEyePoses are used for rendering, they should be passed to ovr_SubmitFrame in ovrLayerEyeFov::RenderPose or ovrLayerEyeFovDepth::RenderPose.
//========================================================================================================================================================================================================================
#undef ovr_CalcEyePoses
OVR_PUBLIC_FUNCTION(void) ovr_CalcEyePoses(ovrPosef headPose, const ovrVector3f hmdToEyeOffset[2], ovrPosef outEyePoses[2]);
OVR_PRIVATE_FUNCTION(void) ovr_CalcEyePoses2(ovrPosef headPose, const ovrPosef HmdToEyePose[2], ovrPosef outEyePoses[2]);
...
//  -hmd Specifies an ovrSession previously returned by ovr_Create.
//  -frameIndex Specifies the targeted frame index, or 0 to refer to one frame after the last time ovr_SubmitFrame was called.
...
//  -hmdToEyePose Can be ovrEyeRenderDesc.HmdToEyePose returned from ovr_GetRenderDesc. For monoscopic rendering, use a position vector that is average of the two position vectors for each eyes.
//  -outEyePoses[out] The predicted eye poses.
//  -outSensorSampleTime[out] The time when this function was called. May be NULL, in which case it is ignored.
//========================================================================================================================================================================================================================
#undef ovr_GetEyePoses
OVR_PUBLIC_FUNCTION(void) ovr_GetEyePoses(ovrSession session, long long frameIndex, ovrBool latencyMarker, const ovrVector3f hmdToEyeOffset[2], ovrPosef outEyePoses[2], double* outSensorSampleTime);
OVR_PRIVATE_FUNCTION(void) ovr_GetEyePoses2( ovrSession session, long long frameIndex, ovrBool latencyMarker, const ovrPosef HmdToEyePose[2], ovrPosef outEyePoses[2], double* outSensorSampleTime);
#define ovr_GetEyePoses ovr_GetEyePoses2

//========================================================================================================================================================================================================================
// Tracking poses provided by the SDK come in a right-handed coordinate system. If an application is passing in ovrProjection_LeftHanded into ovrMatrix4f_Projection, then it should also use
// this function to flip the HMD tracking poses to be left-handed. While this utility function is intended to convert a left-handed ovrPosef into a right-handed coordinate system, 
// it will also work for converting right-handed to left-handed since the flip operation is the same for both cases.
//  -inPose that is right-handed
//  -outPose[out] that is requested to be left-handed (can be the same pointer to inPose)
//========================================================================================================================================================================================================================
OVR_PUBLIC_FUNCTION(void) ovrPosef_FlipHandedness(const ovrPosef* inPose, ovrPosef* outPose);

//========================================================================================================================================================================================================================
// Reads an audio channel from Wav (Waveform Audio File) data. Input must be a byte buffer representing a valid Wav file. Audio samples from the specified
// channel are read, converted to float [-1.0f, 1.0f] and returned through ovrAudioChannelData.
// Supported formats: PCM 8b, 16b, 32b and IEEE float (little-endian only).
//  -outAudioChannel[out] output audio channel data.
...
//  -outHapticsClip[out] generated Haptics clip.
//  -audioChannel input audio channel data.
//  -genMode mode used to convert and audio channel data to Haptics data.
//========================================================================================================================================================================================================================
