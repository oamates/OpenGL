...
// We don't use version numbers within OVR_CAPI_h, as all versioned variations of this file are currently mutually exclusive.
#ifndef OVR_CAPI_h
#define OVR_CAPI_h
...
// ***** OVR_CC_HAS_FEATURE
//
// This is a portable way to use compile-time feature identification available with some compilers in a clean way. 
// Direct usage of __has_feature in preprocessing statements of non-supporting compilers results in a preprocessing error.
//
// Example usage:
...
//========================================================================================================================================================================================================================
// ***** OVR_STATIC_ASSERT
// Portable support for C++11 static_assert(). Acts as if the following were declared:
...
//========================================================================================================================================================================================================================
        ...
        && !(defined(_MSC_VER) && (_MSC_VER >= 1600) && defined(__cplusplus)) /* VS2010+  */
            ...
            #define OVR_STATIC_ASSERT(expression, msg) typedef char OVR_SA_HELP(staticAssert, __COUNTER__)[((expression) != 0) ? 1 : -1] OVR_SA_UNUSED
        #else
            #define OVR_STATIC_ASSERT(expression, msg) typedef char OVR_SA_HELP(staticAssert, __LINE__)[((expression) != 0) ? 1 : -1] OVR_SA_UNUSED
        #endif
    #else
...
//========================================================================================================================================================================================================================
// ***** Padding
// Defines explicitly unused space for a struct. When used correcly, usage of this macro should not change the size of the struct.
...
//========================================================================================================================================================================================================================
typedef struct OVR_ALIGNAS(4) ovrColorf_ { float r, g, b, a; } ovrColorf;           // RGBA color with normalized float components.
typedef struct OVR_ALIGNAS(4) ovrVector2i_ { int x, y; } ovrVector2i;               // 2D vector with integer components.
typedef struct OVR_ALIGNAS(4) ovrSizei_ { int w, h; } ovrSizei;                     // A 2D size with integer components.

typedef struct OVR_ALIGNAS(4) ovrRecti_                                             // 2D rectangle with a integer position and size.
{
    ovrVector2i Pos;
    ovrSizei Size;
} ovrRecti;

typedef struct OVR_ALIGNAS(4) ovrQuatf_ { float x, y, z, w; } ovrQuatf;             // A quaternion rotation.
typedef struct OVR_ALIGNAS(4) ovrVector2f_ { float x, y; } ovrVector2f;             // A 2D vector with float components.
typedef struct OVR_ALIGNAS(4) ovrVector3f_ { float x, y, z; } ovrVector3f;          // A 3D vector with float components.
typedef struct OVR_ALIGNAS(4) ovrMatrix4f_ { float M[4][4]; } ovrMatrix4f;          // A 4x4 matrix with float elements.

typedef struct OVR_ALIGNAS(4) ovrPosef_                                             // Position and orientation together. The coordinate system used is right-handed Cartesian.
{
...
typedef enum ovrHmdType_
{
    ...
    ovrHmd_ES11 = 13,
    ovrHmd_CV1 = 14,
    ovrHmd_EnumSize = 0x7fffffff                                                    // internal, to force type int32_t
} ovrHmdType;
...
{
    // Read-only flags
    ovrHmdCap_DebugDevice = 0x0010,                                                 // Specifies that the HMD is a virtual debug device.
    ovrHmdCap_EnumSize = 0x7fffffff                                                 // internal, to force type int32_t
} ovrHmdCaps;
...
typedef enum ovrTrackingCaps_ 
{
    ...
    ovrTrackingCap_EnumSize = 0x7fffffff                                            // internal, to force type int32_t
} ovrTrackingCaps;
...
typedef enum ovrEyeType_
{
    ...
    ovrEye_EnumSize = 0x7fffffff                                                    // internal, to force type int32_t
} ovrEyeType;
...
typedef enum ovrTrackingOrigin_
{
    ...
    // Prefer using this origin when your application requires matching user's current physical head pose to a virtual head pose
    // without any regards to a the height of the floor. Cockpit-based, or 3rd-person experiences are ideal candidates.
    ...
    // When used, all poses in ovrTrackingState are reported as an offset transform from the profile calibrated floor pose. 
    // Calling ovr_RecenterTrackingOrigin will recenter the X & Z axes as well as yaw, but the Y-axis (i.e. height) will continue
    ...
    ovrTrackingOrigin_EnumSize = 0x7fffffff                                         // internal, to force type int32_t
} ovrTrackingOrigin;
...
{
    char Reserved[8];                                                               // Public definition reserves space for graphics API-specific implementation
} ovrGraphicsLuid;
...
// The values must be the same as in enum StatusBits, see ovrTrackingState
//========================================================================================================================================================================================================================
typedef enum ovrStatusBits_ 
{
    ...
    ovrStatus_EnumSize = 0x7fffffff                                                 // internal, to force type int32_t
} ovrStatusBits;

//========================================================================================================================================================================================================================
// Specifies the description of a single sensor, see ovr_GetTrackerDesc
//========================================================================================================================================================================================================================
...
//========================================================================================================================================================================================================================
// Specifies sensor flags, see ovrTrackerPose
//========================================================================================================================================================================================================================
...
// Returned by ovr_GetTrackingState, see ovr_GetTrackingState
//========================================================================================================================================================================================================================
typedef struct OVR_ALIGNAS(8) ovrTrackingState_
{
    ovrPoseStatef HeadPose;                                                         // Predicted head pose (and derivatives) at the requested absolute time.
    unsigned int StatusFlags;                                                       // HeadPose tracking status described by ovrStatusBits.
    ...
    // The pose of the origin captured during calibration. Like all other poses here, this is expressed in the space set by ovr_RecenterTrackingOrigin,
    // or ovr_SpecifyTrackingOrigin and so will change every time either of those functions are called. This pose can be used to calculate where 
    // the calibrated origin lands in the new recentered space. If an application never calls ovr_RecenterTrackingOrigin or ovr_SpecifyTrackingOrigin, 
    // expect this value to be the identity pose and as such will point respective origin based on ovrTrackingOrigin requested when calling ovr_GetTrackingState.
    ovrPosef CalibratedOrigin;
} ovrTrackingState;

//========================================================================================================================================================================================================================
// Rendering information for each eye. Computed by ovr_GetRenderDesc() based on the specified FOV. Note that the rendering viewport is not included here 
// as it can be specified separately and modified per frame by passing different Viewport values in the layer structure, see ovr_GetRenderDesc
//========================================================================================================================================================================================================================
...
// - HmdToEyePose is the same value pair provided in ovrEyeRenderDesc.
// - HmdSpaceToWorldScaleInMeters is used to scale player motion into in-application units.
// In other words, it is how big an in-application unit is in the player's physical meters. For example, if the application uses inches as
// its units then HmdSpaceToWorldScaleInMeters would be 0.0254. Note that if you are scaling the player in size, this must also scale. 
...
// The type of texture resource, see ovrTextureSwapChainDesc
//========================================================================================================================================================================================================================
typedef enum ovrTextureType_
{
    ...
    ovrTexture_Count,
    ovrTexture_EnumSize = 0x7fffffff                                                // internal, to force type int32_t
} ovrTextureType;
...
typedef enum ovrTextureBindFlags_
{
    ...
    ovrTextureBind_EnumSize = 0x7fffffff                                            // internal, to Force type int32_t.
} ovrTextureBindFlags;

//========================================================================================================================================================================================================================
// The format of a texture, see ovrTextureSwapChainDesc
//========================================================================================================================================================================================================================
typedef enum ovrTextureFormat_
{
    ...
    OVR_FORMAT_ENUMSIZE = 0x7fffffff                                                // internal, to force type int32_t
} ovrTextureFormat;

//========================================================================================================================================================================================================================
// Misc flags overriding particular behaviors of a texture swap chain, see ovrTextureSwapChainDesc
//========================================================================================================================================================================================================================
typedef enum ovrTextureMiscFlags_
{
    ...
    // Texture swap chain contains protected content, and requires HDCP connection in order to display to HMD. Also prevents
    // mirroring or other redirection of any frame containing this contents
    ovrTextureMisc_ProtectedContent = 0x0004,

    ovrTextureMisc_EnumSize = 0x7fffffff                                            // internal, to force type int32_t
} ovrTextureFlags;
...
// See ovr_CreateMirrorTextureDX / ovr_CreateMirrorTextureWithOptionsDX /  ovr_CreateMirrorTextureGL / ovr_CreateMirrorTextureWithOptionsGL / ovr_CreateMirrorTextureWithOptionsVk
//========================================================================================================================================================================================================================
...
{ 
    ...
    ovrButton_EnumSize = 0x7fffffff                                                 // internal, to force type int32_t
} ovrButton;

//========================================================================================================================================================================================================================
// Describes touch input types. These values map to capacitive touch values reported ovrInputState::Touch.
...
typedef enum ovrTouch_
{
    ...
    ovrTouch_EnumSize = 0x7fffffff                                                  // internal, to force type int32_t
} ovrTouch;
...
{
    int SampleRateHz;                                                               // Haptics engine frequency/sample-rate, sample time in seconds equals 1.0/sampleRateHz    
    int SampleSizeInBytes;                                                          // Size of each Haptics sample, sample value range is [0, 2^(Bytes*8)-1]
...
typedef enum ovrControllerType_ 
{
    ...
    ovrControllerType_EnumSize = 0x7fffffff                                         // internal. to force type int32_t
} ovrControllerType;
...
{  
    ovrHapticsBufferSubmit_Enqueue                                                  // Enqueue buffer for later playback
} ovrHapticsBufferSubmitMode;

#define OVR_HAPTICS_BUFFER_SAMPLES_MAX 256                                          /* Maximum number of samples in ovrHapticsBuffer */
...
{
    const void* Samples;                                                            // Samples stored in opaque format
    int SamplesCount;                                                               // Number of samples (up to OVR_HAPTICS_BUFFER_SAMPLES_MAX)
    ovrHapticsBufferSubmitMode SubmitMode;                                          // How samples are submitted to the hardware
} ovrHapticsBuffer;
...
{
    ovrBoundary_Outer = 0x0001,                                                     // Outer boundary - closely represents user setup walls
    ovrBoundary_PlayArea = 0x0100,                                                  // Play area - safe rectangular area inside outer boundary which can optionally be used to  restrict user interactions and motion.
} ovrBoundaryType;
...
{
    ovrBool IsTriggering;                                                           // True if the boundary system is being triggered. Note that due to fade in/out effects this may not exactly match visibility.
...
//========================================================================================================================================================================================================================
// Provides names for the left and right hand array indexes. See ovrInputState, ovrTrackingState
//========================================================================================================================================================================================================================
typedef enum ovrHandType_
{
    ...
    ovrHand_Right = 1,
    ovrHand_Count = 2,
    ovrHand_EnumSize = 0x7fffffff                                                   // internal, to force type int32_t
} ovrHandType;

//========================================================================================================================================================================================================================
// ovrInputState describes the complete controller input state, including Oculus Touch,
// and XBox gamepad. If multiple inputs are connected and used at the same time, their inputs are combined.
//========================================================================================================================================================================================================================
typedef struct ovrInputState_
{
    ...
    // Left and right finger trigger values (ovrHand_Left and ovrHand_Right), in range 0.0 to 1.0f. Returns 0 if the value would otherwise be less than 0.1176, 
    // for ovrControllerType_XBox. This has been formally named simply "Trigger". We retain the name IndexTrigger for backwards code compatibility.
    // User-facing documentation should refer to it as the Trigger.
    float IndexTrigger[ovrHand_Count];
    // Left and right hand trigger values (ovrHand_Left and ovrHand_Right), in the range 0.0 to 1.0f. This has been formally named "Grip Button". 
    // We retain the name HandTrigger for backwards code compatibility.
    ...
    // Left and right finger trigger values (ovrHand_Left and ovrHand_Right), in range 0.0 to 1.0f. Does not apply a deadzone. Only touch applies a filter.
    // This has been formally named simply "Trigger". We retain the name IndexTrigger for backwards code compatibility.
    // User-facing documentation should refer to it as the Trigger.
    float IndexTriggerNoDeadzone[ovrHand_Count];
    // Left and right hand trigger values (ovrHand_Left and ovrHand_Right), in the range 0.0 to 1.0f. Does not apply a deadzone. Only touch applies a filter.
    ...
    // Left and right finger trigger values (ovrHand_Left and ovrHand_Right), in range 0.0 to 1.0f.No deadzone or filter
    // This has been formally named "Grip Button". We retain the name HandTrigger for backwards code compatibility.
    // User-facing documentation should refer to it as the Grip Button or simply Grip.
    float IndexTriggerRaw[ovrHand_Count];
    // Left and right hand trigger values (ovrHand_Left and ovrHand_Right), in the range 0.0 to 1.0f. No deadzone or filter
    // This has been formally named "Grip Button". We retain the name HandTrigger for backwards code compatibility.
    // User-facing documentation should refer to it as the Grip Button or simply Grip.
...
{
    ovrCameraStatus_None = 0x0,                                                     // Initial state of camera
    ovrCameraStatus_Connected = 0x1,                                                // Bit set when the camera is connected to the system
    ovrCameraStatus_Calibrating = 0x2,                                              // Bit set when the camera is undergoing calibration
    ovrCameraStatus_CalibrationFailed = 0x4,                                        // Bit set when the camera has tried & failed calibration
    ovrCameraStatus_Calibrated = 0x8,                                               // Bit set when the camera has tried & passed calibration
    ovrCameraStatus_EnumSize = 0x7fffffff                                           // internal, to force type int32_t
...
typedef struct ovrCameraExtrinsics_
{
    ...
    // The relative Pose of the External Camera. If AttachedToDevice is ovrTrackedDevice_None, then this is a absolute pose in tracking space
...
// Initialization flags, see ovrInitParams, ovr_Initialize
//========================================================================================================================================================================================================================
typedef enum ovrInitFlags_
{
    ...
    ovrInit_Invisible = 0x00000010,                                                 // This client will not be visible in the HMD. Typically set by diagnostic or debugging utilities.
    ovrInit_MixedRendering = 0x00000020,                                            // This client will alternate between VR and 2D rendering.
    ovrinit_WritableBits = 0x00ffffff,                                              // These bits are writable by user code.
    ovrInit_EnumSize = 0x7fffffff                                                   // internal, to force type int32_t
} ovrInitFlags;
...
typedef enum ovrLogLevel_ 
{
    ...
    ovrLogLevel_EnumSize = 0x7fffffff                                               // internal, to force type int32_t
} ovrLogLevel;
...
//========================================================================================================================================================================================================================
// Parameters for ovr_Initialize, see ovr_Initialize
//========================================================================================================================================================================================================================
typedef struct OVR_ALIGNAS(8) ovrInitParams_ 
{
    ...
    // Flags must include ovrInit_RequestVersion or this will be ignored and OVR_MINOR_VERSION will be used. 
    // If you are directly calling the LibOVRRT version of ovr_Initialize in the LibOVRRT DLL then this must be valid and include ovrInit_RequestVersion.
    ...
    OVR_ON64(OVR_UNUSED_STRUCT_PAD(pad0, 4))                                        // internal padding
} ovrInitParams;
...
// Initializes LibOVR
//
// Initialize LibOVR for application usage. This includes finding and loading the LibOVRRT shared library.
// No LibOVR API functions, other than ovr_GetLastErrorInfo and ovr_Detect, can be called unless ovr_Initialize succeeds. 
// A successful call to ovr_Initialize must be eventually followed by a call to ovr_Shutdown. ovr_Initialize calls are idempotent.
// Calling ovr_Initialize twice does not require two matching calls to ovr_Shutdown. If already initialized, the return value is ovr_Success.
//
...
//
//  -params Specifies custom initialization options. May be NULL to indicate default options when using the CAPI shim. 
//          If you are directly calling the LibOVRRT version of ovr_Initialize in the LibOVRRT DLL then this must be valid and include ovrInit_RequestVersion.
//  -return Returns an ovrResult indicating success or failure. In the case of failure, use ovr_GetLastErrorInfo to get more information. Example failed results include:
...
// After calling ovr_Shutdown, no LibOVR functions can be called except ovr_GetLastErrorInfo or another ovr_Initialize. 
// ovr_Shutdown invalidates all pointers, references, and created objects previously returned by LibOVR functions. 
// The LibOVRRT shared library can be unloaded by ovr_Shutdown.
//========================================================================================================================================================================================================================
...
// To avoid any inconsistency, ovr_GetLastErrorInfo should be called immediately after an API function that returned a failed ovrResult, 
// with no other API functions called in the interim.
//
//  -errorInfo[out] The last ovrErrorInfo for the current thread.
// See ovrErrorInfo
//========================================================================================================================================================================================================================
...
// The string is one or more newline-delimited lines of optional info indicating engine name, engine version, engine plugin name, engine plugin version, engine editor.
// The order of the lines is not relevant. Individual lines are optional. A newline is not necessary at the end of the last line. 
// Call after ovr_Initialize and before the first call to ovr_Create. Each value is limited to 20 characters.
// Key names such as 'EngineName:' 'EngineVersion:' do not count towards this limit.
//
...
//
//  -session Specifies an ovrSession previously returned by ovr_Create() or NULL.
//  -return Returns an ovrHmdDesc. If invoked with NULL session argument, ovrHmdDesc::Type set to ovrHmd_None indicates that the HMD is not connected.
//========================================================================================================================================================================================================================
...
// ovr_Initialize must have first been called in order for this to succeed, otherwise the returned trackerDescArray will be zero-initialized. 
// The data returned by this function can change at runtime.
//
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -trackerDescIndex Specifies a tracker index. The valid indexes are in the range of 0 to the tracker count returned by ovr_GetTrackerCount.
...
//  -session Specifies an ovrSession previously returned by ovr_Create, see ovr_Create
//========================================================================================================================================================================================================================
...
//========================================================================================================================================================================================================================
// Specifies status information for the current session, see ovr_GetSessionStatus
//========================================================================================================================================================================================================================
...
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -sessionStatus Provides an ovrSessionStatus that is filled in.
...
// This resets the (x,y,z) positional components and the yaw orientation component of the tracking space for the HMD and controllers using the HMD's current tracking pose.
// If the caller requires some tweaks on top of the HMD's current tracking pose, consider using ovr_SpecifyTrackingOrigin instead.
// The roll and pitch orientation components are always determined by gravity and cannot be redefined. All future tracking will report values relative to this new reference position.
...
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -return Returns an ovrResult indicating success or failure. In the case of failure, use ovr_GetLastErrorInfo to get more information. Return values include but aren't limited to:
//     - ovrSuccess: Completed successfully.
...
// This function is similar to ovr_RecenterTrackingOrigin in that it modifies the (x,y,z) positional components and the yaw orientation component of the tracking space for the 
// HMD and controllers. While ovr_RecenterTrackingOrigin resets the tracking origin in reference to the HMD's current pose, ovr_SpecifyTrackingOrigin allows the caller to 
// explicitly specify a transform for the tracking origin. This transform is expected to be an offset to the most recent recentered origin, so calling this function repeatedly 
// with the same originPose will keep nudging the recentered origin in that direction.
// There are several use cases for this function. For example, if the application decides to limit the yaw, or translation of the recentered pose instead of directly using the 
// HMD pose the application can query the current tracking state via ovr_GetTrackingState, and apply some limitations to the HMD pose because feeding this pose back into this 
// function.  Similarly, this can be used to "adjust the seating position" incrementally in apps that feature seated experiences such as cockpit-based games.
...
// The roll and pitch orientation components are determined by gravity and cannot be redefined. If you are using ovrTrackerPoses then you will need to call ovr_GetTrackerPose 
...
//     - ovrSuccess: Completed successfully.
//     - ovrError_InvalidParameter: The heading direction in originPose was invalid, such as facing vertically. 
//                                   This can happen if the caller is directly feeding the pose of a position-tracked device such as an HMD or controller into this function.
...
// Clears the ShouldRecenter status bit in ovrSessionStatus, allowing further recenter requests to be detected. Since this is automatically done by ovr_RecenterTrackingOrigin and
// ovr_SpecifyTrackingOrigin, this function only needs to be called when application is doing its own re-centering logic.
//========================================================================================================================================================================================================================
...
//  -latencyMarker Specifies that this call is the point in time where the "App-to-Mid-Photon" latency timer starts from. If a given ovrLayer
...
//  -inputState Input state that will be filled in.
...
//========================================================================================================================================================================================================================
OVR_PUBLIC_FUNCTION(ovrResult) ovr_TestBoundary(ovrSession session, ovrTrackedDeviceType deviceBitmask, ovrBoundaryType boundaryType, ovrBoundaryTestResult* outTestResult);
...
//  -outTestResult[out] Result of collision/proximity test, contains information such as distance and closest point.
//  -return Returns an ovrResult for which OVR_SUCCESS(result) is false upon error and true upon success. Return values include but aren't limited to:
//     - ovrSuccess: The call succeeded and a result was returned.
//     - ovrSuccess_BoundaryInvalid: The call succeeded but the result is not a valid boundary due to not being set up.
// See ovrBoundaryTestResult
...
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -boundaryType Must be either ovrBoundary_Outer or ovrBoundary_PlayArea.
//  -outFloorPoints[out] Array of 3D points (in clockwise order) defining the boundary at floor height (can be NULL to retrieve only the number of points).
//  -outFloorPointsCount[out] Number of 3D points returned in the array.
//  -return Returns an ovrResult for which OVR_SUCCESS(result) is false upon error and true upon success. Return values include but aren't limited to:
//     - ovrSuccess: The call succeeded and a result was returned.
//     - ovrSuccess_BoundaryInvalid: The call succeeded but the result is not a valid boundary due to not being set up.
//========================================================================================================================================================================================================================
...
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -boundaryType Must be either ovrBoundary_Outer or ovrBoundary_PlayArea.
//  -outDimensions[out] Dimensions of the axis aligned bounding box that encloses the area in meters (width, height and length).
//  -return Returns an ovrResult for which OVR_SUCCESS(result) is false upon error and true upon success. Return values include but aren't limited to:
//     - ovrSuccess: The call succeeded and a result was returned.
//     - ovrSuccess_BoundaryInvalid: The call succeeded but the result is not a valid boundary due to not being set up.
//========================================================================================================================================================================================================================
...
// Note: visibility is false if the user has turned off boundaries, otherwise, it's true if the app has requested boundaries to be visible or 
// if any tracked device is currently triggering it. This may not exactly match rendering due to fade-in and fade-out effects.
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -outIsVisible[out] ovrTrue, if the boundary is visible.
//  -return Returns an ovrResult for which OVR_SUCCESS(result) is false upon error and true upon success. Return values include but aren't limited to:
...
// ***** Layers
//========================================================================================================================================================================================================================
enum { ovrMaxLayerCount = 16 };                                                     //  Specifies the maximum number of layers supported by ovr_SubmitFrame.
...
typedef enum ovrLayerType_
{
    ...
    ovrLayerType_EnumSize = 0x7fffffff                                              // internal, to force type int32_t
} ovrLayerType;

//========================================================================================================================================================================================================================
// Identifies flags used by ovrLayerHeader and which are passed to ovr_SubmitFrame, see ovrLayerHeader
//========================================================================================================================================================================================================================
typedef enum ovrLayerFlags_ 
{
    // ovrLayerFlag_HighQuality enables 4x anisotropic sampling during the composition of the layer. The benefits are mostly visible at the periphery for high-frequency & 
    // high-contrast visuals. For best results consider combining this flag with an ovrTextureSwapChain that has mipmaps and instead of using arbitrary sized textures, 
    // prefer texture sizes that are powers-of-two. Actual rendered viewport and doesn't necessarily have to fill the whole texture.
    ...
    // Mark this surface as "headlocked", which means it is specified relative to the HMD and moves with it, rather than being specified relative to sensor/torso space and remaining still 
    // while the head moves. What used to be ovrLayerType_QuadHeadLocked is now ovrLayerType_Quad plus this flag. However the flag can be applied to any layer type to achieve a similar effect.
    ovrLayerFlag_HeadLocked = 0x04
} ovrLayerFlags;

//========================================================================================================================================================================================================================
// Defines properties shared by all ovrLayer structs, such as ovrLayerEyeFov. ovrLayerHeader is used as a base member in these larger structs.
// This struct cannot be used by itself except for the case that Type is ovrLayerType_Disabled. See ovrLayerType, ovrLayerFlags
//========================================================================================================================================================================================================================
...
//========================================================================================================================================================================================================================
// Describes a layer that specifies a monoscopic or stereoscopic view. This is the kind of layer that's typically used as layer 0 to ovr_SubmitFrame,
...
{
    ovrLayerHeader Header;    /// Header.Type must be ovrLayerType_EyeFov.
    ovrTextureSwapChain ColorTexture[ovrEye_Count];   /// ovrTextureSwapChains for the left and right eye respectively. The second one of which can be NULL for cases described above.
    ovrRecti Viewport[ovrEye_Count];
    ovrFovPort Fov[ovrEye_Count];                     /// The viewport field of view.

    // Specifies the position and orientation of each eye view, with position specified in meters. RenderPose will typically be the value returned from ovr_CalcEyePoses,
    // but can be different in special cases if a different head pose is used for rendering.
    ovrPosef RenderPose[ovrEye_Count];

    // Specifies the timestamp when the source ovrPosef (used in calculating RenderPose) was sampled from the SDK. Typically retrieved by calling ovr_GetTimeInSeconds
    // around the instant the application calls ovr_GetTrackingState. The main purpose for this is to accurately track app tracking latency.
    double SensorSampleTime;
} ovrLayerEyeFov;


//========================================================================================================================================================================================================================
// Describes a layer that specifies a monoscopic or stereoscopic view. This uses a direct 3x4 matrix to map from view space to the UV coordinates.
...
typedef struct OVR_ALIGNAS(OVR_PTR_SIZE) ovrLayerEyeMatrix_
{
    ...
    ovrTextureSwapChain ColorTexture[ovrEye_Count];                                 // ovrTextureSwapChains for the left and right eye respectively. The second one of which can be NULL for cases described above.
    ovrRecti Viewport[ovrEye_Count];                                                // Specifies the ColorTexture sub-rect UV coordinates. Both Viewport[0] and Viewport[1] must be valid.
...
//========================================================================================================================================================================================================================
// Describes a layer of Quad type, which is a single quad in world or viewer space. It is used for ovrLayerType_Quad. This type of layer represents a single
// object placed in the world and not a stereo view of the world itself.
// A typical use of ovrLayerType_Quad is to draw a television screen in a room that for some reason is more convenient to draw as a layer than as part of the main
// view in layer 0. For example, it could implement a 3D popup GUI that is drawn at a higher resolution than layer 0 to improve fidelity of the GUI.
// Quad layers are visible from both sides; they are not back-face culled.
// See ovrTextureSwapChain, ovr_SubmitFrame
//========================================================================================================================================================================================================================
typedef struct OVR_ALIGNAS(OVR_PTR_SIZE) ovrLayerQuad_
{
    ovrLayerHeader Header;                                                          // Header.Type must be ovrLayerType_Quad.
    ...
    // Specifies the orientation and position of the center point of a Quad layer type. The supplied direction is the vector perpendicular to the quad.
...
// These functions support rendering of distortion by the SDK. TextureSwapChain creation is rendering API-specific.
...
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -chain Specifies the ovrTextureSwapChain for which the length should be retrieved.
//  -out_Length[out] Returns the number of buffers in the specified chain.
...
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -chain Specifies the ovrTextureSwapChain for which the index should be retrieved.
//  -out_Index[out] Returns the current (free) index in specified chain.
//  -return Returns an ovrResult for which OVR_SUCCESS(result) is false upon error.
// See ovr_CreateTextureSwapChainDX, ovr_CreateTextureSwapChainGL
//========================================================================================================================================================================================================================
...
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -chain Specifies the ovrTextureSwapChain for which the description should be retrieved.
//  -out_Desc[out] Returns the description of the specified chain.
//  -return Returns an ovrResult for which OVR_SUCCESS(result) is false upon error.
// See ovr_CreateTextureSwapChainDX, ovr_CreateTextureSwapChainGL
//========================================================================================================================================================================================================================
...
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -chain Specifies the ovrTextureSwapChain to commit.
// When Commit is called, the texture at the current index is considered ready for use by the runtime, and further writes to it should be avoided. 
// The swap chain's current index is advanced, providing there's room in the chain. The next time the SDK dereferences this texture swap chain, it will synchronize 
// with the app's graphics context and pick up the submitted index, opening up room in the swap chain for further commits.
//  -return Returns an ovrResult for which OVR_SUCCESS(result) is false upon error. Failures include but aren't limited to:
...
//========================================================================================================================================================================================================================
// MirrorTexture creation is rendering API-specific. ovr_CreateMirrorTextureDX and ovr_CreateMirrorTextureGL can be found in the
...
// Higher FOV will generally require larger textures to maintain quality. Apps packing multiple eye views together on the same texture should ensure there are at least 8 pixels 
// of padding between them to prevent texture filtering and chromatic aberration causing images to leak between the two eye views.
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -eye Specifies which eye (left or right) to calculate for.
//  -fov Specifies the ovrFovPort to use.
//  -pixelsPerDisplayPixel Specifies the ratio of the number of render target pixels to display pixels at the center of distortion. 1.0 is the default value. 
//           Lower values can improve performance, higher values give improved quality.
// Example code:
...
//  -viewScaleDesc Provides additional information needed only if layerPtrList contains an ovrLayerType_Quad. 
//                 If NULL, a default version is used based on the current configuration and a 1.0 world scale.
...
//                Each layer header must be a part of a layer structure such as ovrLayerEyeFov or ovrLayerQuad, with Header.Type identifying its type. 
//                A NULL layerPtrList entry in the array indicates the absence of the given layer.
//
...
//      - ovrSuccess_NotVisible: rendering completed successfully but was not displayed on the HMD, usually because another application currently has ownership of the HMD. 
//        Applications receiving this result should stop rendering new content, call ovr_GetSessionStatus to detect visibility.
...
//        the session needs to destroyed (ovr_Destroy) and recreated (ovr_Create), and new resources need to be created (ovr_CreateTextureSwapChainXXX). The application's 
//        existing private graphics resources do not need to be recreated unless the new ovr_Create call returns a different GraphicsLuid.
...
//========================================================================================================================================================================================================================
// Contains the performance stats for a given SDK compositor frame. All of the 'int' typed fields can be reset via the ovr_ResetPerfStats call.
//========================================================================================================================================================================================================================
typedef struct OVR_ALIGNAS(4) ovrPerfStatsPerCompositorFrame_
{
    int HmdVsyncIndex;                              // Vsync Frame Index - increments with each HMD vertical synchronization signal (i.e. vsync or refresh rate) If the compositor drops a frame, expect this value to increment more than 1 at a time.

    // Application stats
    int AppFrameIndex;                              // Index that increments with each successive ovr_SubmitFrame call
    int AppDroppedFrameCount;                       // If the app fails to call ovr_SubmitFrame on time, then expect this value to increment with each missed frame
    float AppMotionToPhotonLatency;                 // Motion-to-photon latency for the application. This value is calculated by either using the SensorSampleTime provided for the ovrLayerEyeFov or if that is not available, then the call to ovr_GetTrackingState which has latencyMarker set to ovrTrue
    float AppQueueAheadTime;                        // Amount of queue-ahead in seconds provided to the app based on performance and overlap of CPU and GPU utilization. A value of 0.0 would mean the CPU & GPU workload is being completed in 1 frame's worth of time, while 11 ms (on the CV1) of queue ahead would indicate that the app's CPU workload for the next frame is overlapping the GPU workload for the current frame.
    float AppCpuElapsedTime;                        // Amount of time in seconds spent on the CPU by the app's render-thread that calls ovr_SubmitFram. Measured as elapsed time between from when app regains control from ovr_SubmitFrame to the next time the app calls ovr_SubmitFrame.
    float AppGpuElapsedTime;                        // Amount of time in seconds spent on the GPU by the app. Measured as elapsed time between each ovr_SubmitFrame call using GPU timing queries.

    // SDK Compositor stats
    int CompositorFrameIndex;                       // Index that increments each time the SDK compositor completes a distortion and timewarp pass. Since the compositor operates asynchronously, even if the app calls ovr_SubmitFrame too late, the compositor will kick off for each vsync.
    int CompositorDroppedFrameCount;                // Increments each time the SDK compositor fails to complete in time. This is not tied to the app's performance, but failure to complete can be related to other factors such as OS capabilities, overall available hardware cycles to execute the compositor in time and other factors outside of the app's control.
    float CompositorLatency;                        // Motion-to-photon latency of the SDK compositor in seconds. This is the latency of timewarp which corrects the higher app latency as well as dropped app frames.
    float CompositorCpuElapsedTime;                 // The amount of time in seconds spent on the CPU by the SDK compositor. Unless the VR app is utilizing all of the CPU cores at their peak performance, there is a good chance the compositor CPU times will not affect the app's CPU performance in a major way.
    float CompositorGpuElapsedTime;                 // The amount of time in seconds spent on the GPU by the SDK compositor. Any time spent on the compositor will eat away from the available GPU time for the app.
    float CompositorCpuStartToGpuEndElapsedTime;    // The amount of time in seconds spent from the point the CPU kicks off the compositor to the point in time the compositor completes the distortion & timewarp on the GPU. In the event the GPU time is not available, expect this value to be -1.0f.
    float CompositorGpuEndToVsyncElapsedTime;       // The amount of time in seconds left after the compositor is done on the GPU to the associated / V-Sync time. In the event the GPU time is not available, expect this value to be -1.0f.

    // Async Spacewarp stats (ASW)
    ovrBool AswIsActive;                            // Will be true if ASW is active for the given frame such that the application is being forced into half the frame-rate while the compositor continues to run at full frame-rate.
...
enum { ovrMaxProvidedFrameStats = 5 };                                              // Maximum number of frames of performance stats provided back to the caller of ovr_GetPerfStats
...
{
    // FrameStatsCount will have a maximum value set by ovrMaxProvidedFrameStats If the application calls ovr_GetPerfStats at the native refresh rate of the HMD
    // then FrameStatsCount will be 1. If the app's workload happens to force ovr_GetPerfStats to be called at a lower rate, then FrameStatsCount will be 2 or more.
    // If the app does not want to miss any performance data for any frame, it needs to ensure that it is calling ovr_SubmitFrame and ovr_GetPerfStats at a rate that 
    // is at least: "HMD_refresh_rate / ovrMaxProvidedFrameStats". On the Oculus Rift CV1 HMD, this will be equal to 18 times per second.
    ...
    // AdaptiveGpuPerformanceScale is an edge-filtered value that a caller can use to adjust the graphics quality of the application to keep the GPU utilization in check. 
    // The value is calculated as: (desired_GPU_utilization / current_GPU_utilization). As such, when this value is 1.0, the GPU is doing the right amount of work for the app.
    // Lower values mean the app needs to pull back on the GPU utilization. If the app is going to directly drive render-target resolution using this value, then be sure to 
    // take the square-root of the value before scaling the resolution with it. Changing render target resolutions however is one of the many things an app can do increase or 
    // decrease the amount of GPU utilization. Since AdaptiveGpuPerformanceScale is edge-filtered and does not change rapidly (i.e. reports non-1.0 values once every couple of seconds) 
    // the app can make the necessary adjustments and then keep watching the value to see if it has been satisfied.
    ...
    // Contains the Process ID of the VR application the stats are being polled for If an app continues to grab perf stats even when it is not visible, 
    // then expect this value to point to the other VR app that has grabbed focus (i.e. became visible)
...
// If the VR app is trying to make sure the stats returned belong to the same application, the caller can compare the VisibleProcessId with their own process ID. 
// Normally this will be the case if the caller is only calling ovr_GetPerfStats when ovr_GetSessionStatus has IsVisible flag set to be true.
// If the VR app calling ovr_GetPerfStats is actually the one visible in the HMD, then new perf stats will only be populated after a new call to ovr_SubmitFrame.
// That means subsequent calls to ovr_GetPerfStats after the first one without calling ovr_SubmitFrame will receive a FrameStatsCount of zero.
// If the VR app is not visible, or was initially marked as ovrInit_Invisible, then each call to ovr_GetPerfStats will immediately fetch new perf stats from the 
// compositor without a need for the ovr_SubmitFrame call.
// Even though invisible VR apps do not require ovr_SubmitFrame to be called to gather new perf stats, since stats are generated at the native refresh rate 
// of the HMD (i.e. 90 Hz for CV1), calling it at a higher rate than that would be unnecessary.
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -outStats[out] Contains the performance stats for the application and SDK compositor
//  -return Returns an ovrResult for which OVR_SUCCESS(result) is false upon error and true upon success.
...
// Only the integer values such as HmdVsyncIndex, AppDroppedFrameCount etc. will be reset as the other fields such as AppMotionToPhotonLatency 
// are independent timing values updated per-frame.
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -return Returns an ovrResult for which OVR_SUCCESS(result) is false upon error and true upon success.
...
// The application should increment frameIndex for each successively targeted frame, and pass that index to any relevant OVR functions that need to apply to the frame identified 
// by that index. This function is thread-safe and allows for multiple application threads to target their processing to the same displayed frame.
...
//  -ession Specifies an ovrSession previously returned by ovr_Create.
//  -frameIndex Identifies the frame the caller wishes to target. A value of zero returns the next frame index.
...
typedef enum ovrPerfHudMode_
{
    ...
    ovrPerfHud_EnumSize = 0x7fffffff                                                // internal, to force type int32_t
} ovrPerfHudMode;
...
//========================================================================================================================================================================================================================
// Debug HUD is provided to help developers gauge and debug the fidelity of their app's stereo rendering characteristics. Using the provided quad and crosshair guides,
// the developer can verify various aspects such as VR tracking units (e.g. meters), stereo camera-parallax properties (e.g. making sure objects at infinity are rendered
...
//
// The app can modify the visual properties of the stereo guide (i.e. quad, crosshair) using the ovr_SetFloatArray function.
// For a list of tweakable properties, see the OVR_DEBUG_HUD_STEREO_GUIDE_* keys in the OVR_CAPI_Keys.h header file.
//========================================================================================================================================================================================================================
typedef enum ovrDebugHudStereoMode_
{
    ...
    ovrDebugHudStereo_EnumSize = 0x7fffffff                                         // internal, to force type int32_t
...
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -cameras[inout] Pointer to the array. If null and the provided array capacity is sufficient, will return ovrError_NullArrayPointer.
//  -inoutCameraCount[inout] Supply the array capacity, will return the actual # of cameras defined. If *inoutCameraCount is too small, will return ovrError_InsufficientArraySize. 
//  -return Returns the ids of external cameras the system knows about. Returns ovrError_NoExternalCameraInfo if there is not any eternal camera information.
//========================================================================================================================================================================================================================
...
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -name Specifies which camera to set the intrinsics or extrinsics for
//  -intrinsics Contains the intrinsic parameters to set, can be null
//  -extrinsics Contains the extrinsic parameters to set, can be null
...
//========================================================================================================================================================================================================================
// Writes or creates an integer property. If the property wasn't previously a boolean property, it is changed to an integer property.
//  -session Specifies an ovrSession previously returned by ovr_Create.
//  -propertyName The name of the property, which needs to be valid only for the call.
//  -value The value to write.
//  -return Returns true if successful, otherwise false. A false result should only occur if the property name is empty or if the property is read-only.
...
// This is at the bottom of this file because the following is dependent on the declarations above.
//========================================================================================================================================================================================================================
