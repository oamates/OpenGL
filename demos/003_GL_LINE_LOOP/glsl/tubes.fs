...
subroutine(procedural_color) vec3 simplex_turbulence(vec3 position)
{
    const int iterations = 4;
    const vec3 base_color[] = vec3[] 
    (
...
subroutine(procedural_color) vec3 cellular_turbulence(vec3 position)
{
    const int iterations = 4;
    const vec3 base_color[] = vec3[] 
    (
    ...
    for (int i = 0; i < iterations; ++i)
    {
        vec2 c = cellular(position * frequency);
        color += weight * abs(c.x * c.y) * base_color[i];
        frequency *= frequency_factor;
        weight *= weight_factor;
    };
...
subroutine(procedural_color) vec3 mercury(vec3 position)
{
    vec3 q = vec3(snoise(4.0f * position + vec3(0.5f * time,  1.45f,       33.1f)), 
                  snoise(4.0f * position + vec3( 43.9f, 0.5f * time,      -11.5f)), 
                  snoise(4.0f * position + vec3(-17.3f,      -43.7f, 0.5f * time)));

    vec3 tex_coord = 4.0f * position + 0.42f * q;
                                       

    const int iterations = 4;
    const vec3 base_color[] = vec3[] 
    (
        vec3(0.476f, 0.410f, 0.544f),
        vec3(0.511f, 0.475f, 0.559f),
        vec3(0.544f, 0.444f, 0.567f),
        vec3(0.497f, 0.432f, 0.573f),
        vec3(0.434f, 0.425f, 0.479f),
        vec3(0.370f, 0.385f, 0.387f)
    );

    vec3 color = vec3(0.57, 0.51, 0.62);
    
    float weight = 0.77f;
    float weight_factor = 0.44f;
    float time_factor = 1.44f;
    float t = time;
    float frequency = 1.0f;
    float frequency_factor = 2.33f;

    for (int i = 0; i < iterations; ++i)
    {
        color += weight * snoise(frequency * tex_coord - vec3(0.0f, 0.0f, t)) * base_color[i];
        frequency *= frequency_factor;
        weight *= weight_factor;
        t *= time_factor;
    }    
...
subroutine(procedural_color) vec3 plasma(vec3 position)
{
...
subroutine(procedural_color) vec3 cellular_mercury(vec3 position)
{

    vec3 q = vec3(snoise(4.0f * position + vec3(  time,  1.45f,  33.1f)), 
                  snoise(4.0f * position + vec3( 43.9f,   time, -11.5f)), 
                  snoise(4.0f * position + vec3(-17.3f, -43.7f,   time)));
...
subroutine(procedural_color) vec3 marble(vec3 position)
{
...
vec3 flow_color(vec3 position)
{
    ...
    return vec3(0.3f, 0.5f, 0.7f) + vec3(n1 + 0.75f * n2 + 0.5f * n3);
}


void main()
{
    vec3 view_direction = camera_ws - position;
    vec3 light_direction = light_ws - position;
    ...
    vec3 surface_gradient = -0.125f * (Bx * tangent_x + By * tangent_y) / theta;

    vec3 n = normalize(normal + surface_gradient);
    vec3 v = normalize(view_direction);                                                         
    vec3 l = normalize(light_direction);

    float cos_theta = dot(n, l);
    vec3 color = 0.25f * diffuse_color;
...
//        float exponent = 0.25f * specular_exponent();
        ...
        color += 0.45f * pow(cos_alpha, Ns) * vec3(1.0f);
    }
