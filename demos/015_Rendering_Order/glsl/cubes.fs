...
void main()
{
    ...
    vec2 q = vec2(1.0f, 1.0f);
    vec3 normal_ms = vec3(texture2D(texture_sampler, texture_coord)) - vec3(0.5f, 0.5f, 0.0f);
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        float ambient_distance_factor = 2 * clamp(150.0f / (distance * distance), 0.0f, 1.0f);
        float specular_distance_factor = 2 * clamp(150.0f / (distance * distance), 0.0f, 1.0f);
        FragmentColor += material_ambient_color * light_color[i] * lambert_cosine * lambert_cosine * ambient_distance_factor;
		specular_color += material_specular_color * light_color[i] * pow(cos_alpha, 20) * specular_distance_factor;
    };
    FragmentColor += specular_color;
    FragmentColor.w = 0.5 + 0.5 * cnoise(300 * texture_coord);
}
