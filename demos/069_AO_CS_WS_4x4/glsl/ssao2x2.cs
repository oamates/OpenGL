...
void main()
{
    vec2 base = 2.0 * vec2(gl_GlobalInvocationID.xy) + 1.0;
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        ao /= W;
        imageStore(ssao_image, 2 * ivec2(gl_GlobalInvocationID.xy) + ibase_shift[i], vec4(ao, 0.0, 0.0, 0.0));
    }
}
