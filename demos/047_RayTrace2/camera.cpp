...
void Camera::SetYPR(float a_yaw, float a_pitch, float a_roll)
{
    m_yaw = a_yaw;
    m_pitch = a_pitch;
    m_roll = a_roll;
...
void Camera::UpdateYPR(float a_yaw, float a_pitch, float a_roll)
{
    m_yaw += a_yaw;
    m_pitch += a_pitch;
    m_roll += a_roll;
...
void Camera::GetCornerRays(vec4& a_ray00, vec4& a_ray01, vec4& a_ray10, vec4& a_ray11, vec4& a_origin)
{
    vec4 hLeft, hUp, forward;
    m_matrix.GetRow(1, hLeft);
    m_matrix.GetRow(2, hUp);
    m_matrix.GetRow(0, forward);
    hLeft *= (0.5f * m_ar);
    hUp *= 0.5f;
    forward *= m_projDist;
