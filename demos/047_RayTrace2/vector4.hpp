...
template<typename Real> void GetBasis(Vector4<Real> const & a_a0, Vector4<Real> const & a_a1, Vector4<Real>& a_x0, Vector4<Real>& a_x1, Vector4<Real>& a_x2)
{
    ...
    else if (is_a1_zero)                                    // x1 only is zero vector
    {
        a_x0 = a_a0;                                        // Build the basis off a_a0
        a_x0.Normalize();
        a_x1 = Perpendicular(a_x0);                         // Set x1
        a_x2 = Cross(a_x0, a_x1);                           // Find perpendicular vector to x0, x1.
        return;
    ...
    a_x0 = a_a0;                                            // Assign x0
    a_x0.Normalize();
    a_x2 = Cross(a_x0, a_a1);                               // Calculate x2


    if (IsZero(a_x2.LengthSquared()))                       // Test to see if a_a0 and a_a1 are parallel
    {
        a_x1 = Perpendicular(a_x0);                         // Find a perpendicular vector
        a_x2 = Cross(a_x0, a_x1);                           // Calculate x2
        return;
    }
...
//    @   Perpendicular()
//--------------------------------------------------------------------------------
template<typename Real> Vector4<Real> Perpendicular(const Vector4<Real>& a_vector)
{
    Vector4<Real> result;
