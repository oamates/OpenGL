...
template<typename T> struct Array1
{
   ...
   // note: default initial values are zero
    ...
    template<typename InputIterator> void assign_(InputIterator first, InputIterator last, Array1True check)
        { fill(first, last); }

    template<typename InputIterator> void assign_(InputIterator first, InputIterator last, Array1False check)
    {
        unsigned long i = 0;
        for(InputIterator p = first; p != last; ++p, ++i)
        {
            if (i == max_n)
                grow();
            data[i] = *p;
