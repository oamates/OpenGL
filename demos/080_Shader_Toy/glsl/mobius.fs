...
{
    float h = clamp(0.5 + 0.5 * (b - a) / s, 0.0f, 1.0f);
    return mix(b, a, h) - h * (1.0f - h) * s;
}

float smax(float a, float b, float s)
{
    float h = clamp(0.5 + 0.5 * (a - b) / s, 0.0f, 1.0f);
...
//
// By the way, I've explained the process in a hurry with a "near enough is good enough"
// attitude, so if any topology experts out there spot any conceptual errors, mislabling, etc, feel free 
// to let me know.
float Mobius(vec3 q)
{
    ...
    //
    // Partitioning the circular path into 32 (ringNum) cells - or sections, then obtaining the 
    // angle of the center position of that cell. The reason you want that angle is so that you can render 
    ...
    p = abs(abs(p) - .25); // Comment this out, and you'll see what it does.
...
vec3 getNormal(vec3 p, inout float edge, inout float crv, float ef, float t){ 
    ...
    vec2 e = vec2(ef / resolution.y, 0); // Larger epsilon for greater sample spread, thus thicker edges.
...
float n3D(in vec3 p)
{
	...
	vec3 ip = floor(p); 
    p -= ip; 
    vec4 h = vec4(0.0f, s.yz, s.y + s.z) + dot(ip, s);
    p = p * p * (3.0 - 2.0 * p);
...
vec3 envMap(vec3 p)
{
    ...
    float n3D2 = n3D(2.0f * p);
   
    // A bit of fBm.
    float c = n3D(p) * 0.57f + n3D2 * 0.28f + n3D(p * 4.0f) * 0.15f;
    c = smoothstep(0.5f, 1.0f, c); // Putting in some dark space.
...
void main()
{
	...
	// Camera Setup.
	vec3 lk = vec3(0, 0.25, 0);  // "Look At" position.
	vec3 ro = lk + vec3(cos(0.25f * time) * 2.8f, cos(0.5f * time) * sin(0.25f * time) * 0.25f + 0.75f, sin(time * 0.25f) * 3.3f); // Camera position, doubling as the ray origin.
 
    // Light positioning. One is just in front of the camera, and the other is in front of that.
 	vec3 lp = ro + vec3(0.85f, 1.75f, -1.0f);// Put it a bit in front of the camera.
    ...
    // Mouse controls.   
	vec2 ms = vec2(0);
    if (mouse.z > 1.0) ms = (2.0 * mouse.xy - resolution.xy) / resolution.xy;
    ...
    */
	...
	if(t < FAR)
    {
        ...
        if(svObjID<0.5) { // Terrain texturing.
            ...
            texCol = vec3(1, .6, .4);//tex3D(iChannel0, sp*tSize0, sn);
    	...
    	float sh = softShadow(sp, lp, 8.0f);
    	...
    	// Diffuse lighting.
	    float diff = max(dot(sn, ld), 0.0f);
        diff = pow(diff, 4.0f) * 1.5f; // Ramping up the diffuse.
    	
    	// Specular lighting.
	    float spec = pow(max( dot( reflect(-ld, sn), -rd ), 0.0f), 16.0f); 
        ...
        vec3 env = envMap(reflect(rd, sn)) * 2.0f; //envMap(refract(rd, sn, 1./1.3))*3.;
        if(svObjID < 0.5f)
        ...
        sceneCol += (texCol*(diff + 0.15 + vec3(.7, .9, 1) * fre) + env + vec3(0.7f, 0.9f, 1.0f) * spec * 2.0);
        
        // Distance function edging for the Mobius object, and bump mapped edging only for the terrain.
        if(svObjID>.5) sceneCol *= (1.0f - edge * 0.8f);
        else sceneCol *= (1.0f - edge2 * 0.7f);
	...
	FragmentColor = vec4(sqrt(clamp(sceneCol, 0.0f, 1.0f)), 1.0f);
