...
const vec3 SUN_COLOR = vec3(1.0, 1.0, 0.98) * 0.75;
...
void main() 
{
    ...
    vec3 ang = vec3(sin(time0 * 6.0) * 0.1, 0.1, -time0);
    ...
    vec3 dist = p - ori;
    vec3 n = getNormal(p, dot(dist, dist) * (0.1 / resolution.x));
