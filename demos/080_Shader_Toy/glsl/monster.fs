...
void main()
{
    ...
    vec2 m = vec2(0.5);
    if (mouse.z > 0.0) m = mouse.xy / resolution.xy;

    // animation    
    float time00 = time;
    time00 += 15.0*smoothstep(  15.0, 25.0, time );
    time00 += 20.0*smoothstep(  65.0, 80.0, time );
    time00 += 35.0*smoothstep( 105.0, 135.0, time );
    time00 += 20.0*smoothstep( 165.0, 180.0, time );
    time00 += 40.0*smoothstep( 220.0, 290.0, time );
    time00 +=  5.0*smoothstep( 320.0, 330.0, time );
    ...
    if( tmat.z>-0.5 )
    {
        ...
        for( int i=0; i<32; i++ )
        {
            ...
            col += pow(vec3(0.03, 0.47, 0.11), vec3(2.2) ) * dot(rr,nor) * ds;
        }
        col /= 32.0;                                        
        ...
        col += 1.8 * (0.04 + 12.0*fre) * occ * pow(vec3(0.3, 0.1, 0.8), vec3(2.0) ) * rs;
