...
float noise( in vec3 x )
{
    ...
    vec2 rg = texture(iChannel0, (uv + 0.5) * TEXEL_SIZE).yx;
#else
    vec2 uv = (p.xy + vec2(37.0, 17.0) * p.z);
    vec2 rg1 = texture(iChannel0, (uv + vec2(0.5, 0.5)) * TEXEL_SIZE).yx;
    vec2 rg2 = texture(iChannel0, (uv + vec2(1.5, 0.5)) * TEXEL_SIZE).yx;
    vec2 rg3 = texture(iChannel0, (uv + vec2(0.5, 1.5)) * TEXEL_SIZE).yx;
    vec2 rg4 = texture(iChannel0, (uv + vec2(1.5, 1.5)) * TEXEL_SIZE).yx;
...
float noise(in vec2 x)
{
    ...
    return texture(iChannel0, (uv + 118.4) / 256.0).x;
}
...
vec4 mapClouds(vec3 pos)
{
    ...
    res.xyz = mix(vec3(1.0, 0.8, 0.7), vec3(0.08, 0.08, 0.08), res.x);
    res.xyz *= 0.25;
    res.xyz *= 0.5 + 0.5 * smoothstep(-2.0, 1.0, pos.y);    
    return res;
}

vec4 raymarchClouds(vec3 ro, vec3 rd, vec3 bcol, float tmax)
{
    vec4 sum = vec4(0.0);
...
vec3 path(float t)
{
    return vec3(16.0 * cos(0.2 + 0.075 * t), 1.5, 16.0 * sin(0.1 + 0.0825 * t));   
}
...
void main()
{
    ...
    // camera   
    float off = 6.0 * mouse.x / resolution.x;
    float time_c = 2.7 + time + off;
    // time_c = 35.0;
    vec3 ro = path(time_c + 0.0);
    vec3 ta = path(time_c + 1.6);
    // ta.y *= 0.3 + 0.25 * cos(0.11 * time_c);
    ta.y *= 0.35 + 0.25 * sin(0.09 * time_c);
    float roll = 0.3 * sin(1.0 + 0.07 * time_c);
    ...
    if(t > 0.0)
    {
        ...
        vec3 bn = -1.0 + 2.0 * texcube(iChannel0, 0.75 * pos, nor).xyz;
        ...
        float sha = (sun > 0.01) ? softshadow(pos, lig, 0.01, 32.0) : 0.0; 
    ...
    // sun glow
    col += vec3(1.0, 0.6, 0.2) * 0.2 * pow(sun, 2.0) * clamp(2.5 * (rd.y + 0.4), 0.0, 1.0);
