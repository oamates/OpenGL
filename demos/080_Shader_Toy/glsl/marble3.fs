...
vec3 marble_color(vec3 position)
{
    ...
    vec3 color = (n >= 0.6667f) ? mix(marble_color2, marble_color3, 3.0f * (n - 0.666667f)) : 
                 (n >= 0.3333f) ? mix(marble_color1, marble_color2, 3.0f * (n - 0.333333f)) :
...
void main()
{
    ...
    ro.yz *= rot(m.y);
    ro.xz *= rot(m.x + 0.25f * time);
    vec3 ta = vec3(0.0f);
    vec3 ww = normalize(ta - ro);
    vec3 uu = normalize(cross(ww, vec3(0.0f, 1.0f, 0.0f)));
    vec3 vv = normalize(cross(uu, ww));
    vec3 rd = normalize(p.x * uu + p.y * vv + 4.0f * ww) * 0.975f;
    vec2 tmm = iSphere(ro, rd, vec4(0.0f, 0.0f, 0.0f, 1.9f));                           
    ...
    {
        vec3 nor = reflect(rd, 0.5f * (ro + tmm.x * rd));        
