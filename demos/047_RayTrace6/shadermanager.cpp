...
#define vertex_shader   "glsl/quad.vs"
#define fragment_shader "glsl/quad.fs"

// Define des path de toutes les composantes du compute shader
#define compute_shader_header    "glsl/head.cs"
#define compute_shader_common    "glsl/common.cs"
#define compute_shader_octree    "glsl/octree.cs"
#define compute_shader_brdf      "glsl/brdf.cs"
#define compute_shader_intersect "glsl/intersect.cs"
#define compute_shader_raytrace2 "glsl/raytrace2.cs"
#define compute_shader_raytrace  "glsl/raytrace.cs"
#define compute_shader_final     "glsl/final.cs"
...
{
    debug_color_msg(DEBUG_ORANGE_COLOR, "Loading file: %s", parFileName.c_str());
    ...
    if(fileStream.is_open())
    {
        ...
        debug_color_msg(DEBUG_RED_COLOR, "Couldn't load shader source file: %s", parFileName.c_str());
    return fileContent;
}

// Méthode d'écriture d'une string dans un fichier
void WriteFile(const std::string& parFileName, const std::string& parContent)
{
    debug_color_msg(DEBUG_ORANGE_COLOR, "Writing to file: %s", parFileName.c_str());
    std::string fileContent;
    std::fstream fileStream;
    ...
    debug_color_msg(DEBUG_RED_COLOR, "Error opening %s", parContent.c_str());
}
...
void CheckShader(GLuint parShaderID)
{
    ...
    if(InfoLogLength > 1)
    {
        ...
        glGetShaderInfoLog(parShaderID, InfoLogLength, 0, errorMessage);
        debug_color_msg(DEBUG_RED_COLOR, "Shader error: id = %d. Compiler message: ", parShaderID);
        put_msg(errorMessage);
    }
}
...
void CheckProgram(GLuint parProgramID)
{
    ...
    {   
        char errorMessage[InfoLogLength];
        glGetProgramInfoLog(parProgramID, InfoLogLength, 0, errorMessage);
        debug_color_msg(DEBUG_RED_COLOR, "Program linking error: id = %d", parProgramID);
        put_msg(errorMessage);
    }
}
...
void ShaderManager::InjectVec3(GLuint parShaderID, const glm::dvec3& parValue, const std::string& parName)
{
    BindProgram(parShaderID);
...
GLuint ShaderManager::CreateProgramC(int parMaxRecur, int parNbTriangle, int parNbPlan, int parNbQuad, int parNbNoeud, int parNbPrimMax, int nbMat, bool oreille)
{
    ...
    WriteFile("raytracer.cs", computeShader);
    
    const char* csc_str = computeShader.c_str();
    glShaderSource(computeShaderID, 1, &csc_str, 0);
...
GLuint ShaderManager::CreateTexNoeud(const std::vector<node_t>& parValue, int par_nb_prim_max, int par_nb_prim)
{
    debug_color_msg(DEBUG_ORANGE_COLOR, "Creating node texture ...");
    GLuint noeudTex;
    ...
    delete[] noeudData;
    debug_color_msg(DEBUG_ORANGE_COLOR, "Done. Node texture id = %u.", noeudTex);
...
GLuint ShaderManager::CreateTexPrimitive(const std::vector<Primitive>& parValue, int parNbMateriau)
{
    ...
    {
        primData[index * 3 + 0] = 0.5 * prim->type;
        primData[index * 3 + 1] = (prim->index / ((float)parValue.size() - 1));
        primData[index * 3 + 2] = (prim->materiau / ((float)parNbMateriau -1));
        debug_color_msg(DEBUG_ORANGE_COLOR, "Primitive :: {%f, %f, %f}", primData[index * 3 + 0], primData[index * 3 + 1], primData[index * 3 + 2]);
        index++;        
    }
