...
vec3 gpuCellNoise3D(vec3 P)
{
    vec3 shift[27] = vec3[27]
    (
        vec3(-1.0f, -1.0f, -1.0f),
        vec3(-1.0f, -1.0f,  0.0f),
        vec3(-1.0f, -1.0f,  1.0f),
    ...
    for (int idx = 0; idx < 27; ++idx)
    {
        ...
        vec3 L = shift[idx];
        vec3 cell = snoise(Pi + L) * noiseScale + L - Pf;
        float new_dist = dot(cell, cell);
        if (new_dist < dist.x)
        {
            dist.yz = dist.xy;
            dist.x = new_dist;
...
subroutine(procedural_color) vec3 flow_color(vec3 position)
{
    vec2 g1, g2;
