...
vec3 color_func(vec3 position)
{
    return marble_color(position);
}
...
void main()
{
    ...
    vec3 diffuse_color = color_func(scale * position_ws);

    float Bx = dot(color_func(scale * (position_ws + theta * tangent_x_ws)) - color_func(scale * (position_ws - theta * tangent_x_ws)), vec3(1.0f));
    float By = dot(color_func(scale * (position_ws + theta * tangent_y_ws)) - color_func(scale * (position_ws - theta * tangent_y_ws)), vec3(1.0f));

    vec3 surface_gradient = -0.0625f * (Bx * tangent_x_ws + By * tangent_y_ws) / theta;
