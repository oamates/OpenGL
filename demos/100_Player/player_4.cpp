//========================================================================================================================================================================================================================
// DEMO 100.4: OpenGL Video Player based on SDL
//========================================================================================================================================================================================================================
#include <cstdio>
#include <cassert>
#include <cmath>
...
VideoState *global_video_state;                                                                             // Since we only have one decoding thread, the Big Struct can be global in case we need it.
AVPacket flush_pkt;
...
int synchronize_audio(VideoState *is, short *samples, int samples_size, double pts)                         // Add or subtract samples to get a better sync, return new audio buffer size
{
    ...
    if(is->av_sync_type != AV_SYNC_AUDIO_MASTER)
    {
            ...
            else
            {
                ...
                if(fabs(avg_diff) >= is->audio_diff_threshold)
                {
                    ...
                    if(wanted_size < samples_size)
                        samples_size = wanted_size;                                                         // remove samples
                    else if(wanted_size > samples_size)
                    {
                        uint8_t *samples_end, *q;
                        int nb = samples_size - wanted_size;                                                // add samples by copying final sample
        ...
        {
            is->audio_diff_avg_count = 0;                                                                   // difference is TOO big; reset diff stuff
            is->audio_diff_cum = 0;
        }
    }
    return samples_size;
...
int audio_decode_frame(VideoState *is, uint8_t *audio_buf, int buf_size, double *pts_ptr)
{
    ...
    while(1)
    {
        while(is->audio_pkt_size > 0)
        {
            ...
            if(len1 < 0)
            {
                is->audio_pkt_size = 0;                                                                     // if error, skip the frame
                break;
            }
            int data_size = 0;
            if(got_frame)
        ...
        if(pkt->pts != AV_NOPTS_VALUE)                                                                      // if update, update the audio clock w/pts
...
void audio_callback(void *userdata, Uint8 *stream, int len)
{
    ...
    while(len > 0)
    {
        ...
        {
            int audio_size = audio_decode_frame(is, is->audio_buf, sizeof(is->audio_buf), &pts);            // We have already sent all our data; get more
            if(audio_size < 0)
            {
...
void video_refresh_timer(void *userdata)
{
    ...
    if(is->video_st)
    {
        ...
        else
        {
            ...
            double delay = vp->pts - is->frame_last_pts;                                                    // the pts from last time
            if(delay <= 0 || delay >= 1.0)
                delay = is->frame_last_delay;                                                               // if incorrect delay, use previous one
                
            is->frame_last_delay = delay;                                                                   // save for next time
            is->frame_last_pts = vp->pts;
                
            if(is->av_sync_type != AV_SYNC_VIDEO_MASTER)                                                    // update delay to sync to audio if not master source
            {
                double ref_clock = get_master_clock(is);
                double diff = vp->pts - ref_clock;
                double sync_threshold = (delay > AV_SYNC_THRESHOLD) ? delay : AV_SYNC_THRESHOLD;            // Skip or repeat the frame. Take delay into account FFPlay still doesn't know if this is the best guess.
            ...
            double actual_delay = is->frame_timer - (av_gettime() / 1000000.0);                             // computer the REAL delay
...
int queue_picture(VideoState *is, AVFrame *pFrame, double pts)
{
    ...
    if(vp->bmp)
    {
        ...
        sws_scale(is->sws_ctx, (uint8_t const * const *)pFrame->data, pFrame->linesize,                     // Convert the image into YUV format that SDL uses
                                0, is->video_ctx->height, pict.data, pict.linesize);
...
double synchronize_video(VideoState *is, AVFrame *src_frame, double pts)
{
    ...
    double frame_delay = av_q2d(is->video_ctx->time_base);                                                  // update the video clock
    frame_delay += src_frame->repeat_pict * (frame_delay * 0.5);                                            // if we are repeating a frame, adjust clock accordingly
...
int stream_component_open(VideoState *is, int stream_index)
{
    ...
    {
        debug_msg("Couldn't copy codec context");
    ...
    if(codecCtx->codec_type == AVMEDIA_TYPE_AUDIO)
    {
        ...
        wanted_spec.freq = codecCtx->sample_rate;                                                           // Set audio settings from codec info
...
int decode_thread(void *arg)
{
    ...
    av_dump_format(pFormatCtx, 0, is->filename, 0);                                                         // Dump information about file onto standard error output

    for(int i = pFormatCtx->nb_streams; i >=0; --i)                                                         // Find the first video and audio stream
    {
        if(pFormatCtx->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO) video_index = i;
        if(pFormatCtx->streams[i]->codec->codec_type == AVMEDIA_TYPE_AUDIO) audio_index = i;
    }
    ...
    while(1)                                                                                                // main decode loop
    {
        ...
        if(is->seek_req)                                                                                    // seek stuff goes here
        {
            int stream_index= -1;
            ...
            if(av_seek_frame(is->pFormatCtx, stream_index, seek_target, is->seek_flags) < 0)
                debug_msg("%s: error while seeking\n", is->pFormatCtx->filename);
            else
            {
                if(is->audioStream >= 0)
                {
    ...
    while(1)
    {
        ...
        switch(event.type)
        {
            case SDL_KEYDOWN:
                ...
                {
                    case SDLK_LEFT:
                        incr = -10.0; goto do_seek;
                    case SDLK_RIGHT:
                        incr = 10.0; goto do_seek;
                    case SDLK_UP:
                        incr = 60.0; goto do_seek;
                    case SDLK_DOWN:
                        incr = -60.0; goto do_seek;
                    do_seek:
                        if(global_video_state)
                        {
                            double pos = get_master_clock(global_video_state);
                            pos += incr;
        ...
        case SDL_QUIT:
            is->quit = 1;
            SDL_CondSignal(is->audioq.cond);                                                                // If the video has finished playing, then both the picture and audio queues are waiting for more data. 
            SDL_CondSignal(is->videoq.cond);                                                                // Make them stop waiting and terminate normally.
            SDL_Quit();
            return 0;
            break;
