...
vec2 cellular(vec3 P) 
{
    ...
    const float Kzo = 5.0 / 12.0;
    const float jitter = 1.0f;                                                          // smaller values produce more regular patterns
    ...
    vec3 d1a = min(d11, d12);                                                           // Sort out the two smallest distances (F1, F2)
    d12 = max(d11, d12);
