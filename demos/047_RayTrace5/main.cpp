...
struct MyEngine : public SimpleGraphicsEngine
{
    MyEngine();
    ~MyEngine();
    virtual void update();
...
struct MyObject3D : public Object3D
{
...
struct LightObject3D : public MyObject3D
{
    enum RenderMode
    {
        shadow_map,
...
void MyEngine::clearVoxels()
{
    ...
    voxelizer_camera_->render(glm::mat4(), shader_clearvoxels_, -1, 1, -1, 1, 1, -1);
...
void MyEngine::voxelizeScene()
{
    ...
    voxelizer_camera_->render(glm::mat4(), shader_voxelization_, -1, 1, -1, 1, 1, -1);
...
void MyEngine::renderVolume()
{
    ...
    camera_->render(glm::mat4(), shader_worldpositionoutput_);
    ...
    camera_->render(glm::mat4(), shader_simplevolume_);
...
void MyEngine::renderGlobal()
{
    ...
    camera_->render(glm::mat4(), shader_global_);
...
void MyEngine::renderLocalDiffuse()
{
    ...
    camera_->render(glm::mat4(), shader_phong_);
  ...
  //if (!TwEventMouseButtonGLFW(button, action))
    {
        ...
        glm::vec3 from = glm::unProject(glm::vec3(mouse_x_, h - mouse_y_, 0.0f), V, P, glm::vec4(0, 0, w, h));
        glm::vec3 to = glm::unProject(glm::vec3(mouse_x_, h - mouse_y_, 1.0f), V, P, glm::vec4(0, 0, w, h));
...
void MyEngine::keyCallback(GLFWwindow * window, int key, int scancode, int action, int mods)
{
...
{
    // if (tweakbar_) TwDeleteBar(tweakbar_);
...
void MyEngine::updateCameraController(float dt)
{
    ...
    glm::mat4 R = glm::rotate(glm::mat4(), yaw, glm::vec3(0,1,0)) * glm::rotate(glm::mat4(), roll, glm::vec3(1,0,0));
...
template <class T> void MyEngine::Delay(T* input, T end_val, float speed)
{
...
CubeTextureFBO::CubeTextureFBO(int size)
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB, size, size, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
