...
{
    /* We have finished all the triangles, now is time to output to the data file. In the strips data structure, every three ids is a triangle. 
       Now we see whether we can swap, or make a new strip or continue the strip, and output the data accordingly to the data file. */
    ...
    *swap = 0;
    ...
    if (temp4 == NULL)
    {
      /* Data might be mixed and we do not have textures for some of the vertices */
        if ((text) &&  (vt[temp3->face_id] == 0)) cptexture = false;
        if ((norm) && (!cptexture))
        {
        
            if (startnewstrip && orient) /* If we want to keep orientation */
                preserve_strip_orientation_with_normal(output, temp3->face_id + 1, vn[temp3->face_id] + 1, temp2->face_id + 1, vn[temp2->face_id] + 1, temp1->face_id + 1, vn[temp1->face_id] + 1);
            fprintf(output," %d//%d %d//%d %d//%d", temp3->face_id + 1, vn[temp3->face_id] + 1, temp2->face_id + 1, vn[temp2->face_id] + 1, temp1->face_id + 1, vn[temp1->face_id] + 1);
        }
  ...
  if ((temp1 == NULL) || (temp2 == NULL) ||
    ...
    exit_msg("There is an error in the output of the triangles\n");
  ...
  for (x = 6; x < num ; x = x+3)
    {
      ...
      if ((temp1 == NULL ) || (temp2 == NULL) || (temp3 == NULL))
        exit_msg("There is an error in the triangle list \n");
...
void Output_Tri(int id1, int id2, int id3,bool end)
{
    ...
    /* Make sure we do not have an error. There are degeneracies in some of the files */
    if ((id1 == id2) || (id1 == id3) || (id2 == id3))
        exit_msg("Degenerate triangle %d %d %d\n", id1, id2, id3);
...
static int Polygon_Output(P_ADJACENCIES temp, int face_id, int bucket, ListHead *pListHead, bool first, int* swaps, FILE* bands, int color1, int color2, int color3, bool global, bool end)
{
  ListHead *pListFace;
  PF_FACES face;
  ...
  if (face->nPolSize == 3)
      ...
      */
      ...
      if ((other2 != 0) && (other3 != 0))
        ...
        e3 = e2;
            ...
            exit_msg("There is an error in the tri with adj\n");
        ...
        other1 = other3;
            ...
            exit_msg("There is an error in getting the tri with adj\n");
  ...
  else
    {
      ...
      */
      ...
      if (Done(next_face_id,&next_bucket) == NULL)
        exit_msg("We deleted the next face 6 %d %d\n", next_face_id, face_id);
      ...
      return (Polygon_Output(lpListInfo,next_face_id,next_bucket, pListHead, false, swaps,bands,color1,color2,color3,global,end));
        }      
    }
}       
