...
static void P_Triangulate_Quad(int out_edge1,int out_edge2,int in_edge1,
                   int in_edge2,int size,int *index,
                   int reversed,
                   int face_id,ListHead *pListHead, 
                   P_ADJACENCIES temp,
                   int where)
{
    ...
    /* This routine will nonblindly triangulate a quad, meaning that there is a definite input and a definite output edge that we must adhere to. 
       ...
       Index are the edges of the polygon and size is the size of the polygon. Begin is whether we are at the start of a new strip.
       Note that we will not necessarily triangulate the whole quad; maybe we will do half and leave the other half (a triangle) for later. */
  
  
    /* If we do not have an input edge, then we can make our input edge whatever we like, therefore it will be easier to come out on the output edge. In this case the whole quad is done. */
    if (reversed == -1)
    {
    ...
    if (in_edge2 == out_edge1) 
    {
        ...
        Delete_AdjEx(in_edge1, in_edge2, &dummy, &dummy, face_id, &dummy, &dummy, &dummy);
        Delete_AdjEx(out_edge2, in_edge2, &dummy, &dummy, face_id, &dummy, &dummy, &dummy);
        /* Put the new face in the proper bucket of adjacencies. There are 2 edges that need to be checked for the triangle
           that was just outputted. For the output edge we definitely will be decreasing the adjacency, but we must check for the input edge. */
...
static void P_Triangulate_Polygon(int out_edge1,int out_edge2,int in_edge1,
  ...
  */
  ...
  /*    Consecutive cases again, but with the output edge reversed */
  else if (in_edge1 == out_edge2)
    {
      ...
      */
      Delete_AdjEx(in_edge1,in_edge2,&dummy,&dummy,face_id, &dummy,&dummy,&dummy);
      Delete_AdjEx(out_edge1,out_edge2,&dummy,&dummy, face_id,&dummy,&dummy,&dummy);
  ...
  */
      ...
      if (!reversed)
    {
      ...
      */
      ...
      if (temp2 == NULL)
        {
          printf("There is an error finding the next polygon :: next_bucket = %u, face_id = %u.\n",
             next_bucket,face_id);
          exit(0);
        }
...
static void P_Triangulate(int out_edge1,int out_edge2,int in_edge1,
              int in_edge2,int size,int *index,
...
static void Input_Edge(int face_id, int *index, int size, int in_edge1, int in_edge2, 
               ListHead *pListHead,
               int where)
{
