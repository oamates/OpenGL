...
{
    /* Return the vertex that is adjacent to id1, but is not id2, in the list of integers. */
    int x = 0;
...
void Delete_From_ListEx(int id,int *list, int size)
{
    ...
    if (y != (size - 1))
        exit_msg("There is an error in the delete\n");
    *(temp + size - 1) = -1;
    memcpy(list, temp, sizeof(int) * size);  
}


static void Triangulate_QuadEx(int out_edge1,int out_edge2,int in_edge1,
...
static void TriangulateEx(int out_edge1,int out_edge2,int in_edge1,
              int in_edge2,int size,int *index,
...
void Blind_TriangulateEx(int size, int *index, bool begin, int where )
{
  ...
  /* Rearrange the index list so that the input edge is first */
    if (!begin) Rearrange_IndexEx(index,size);
