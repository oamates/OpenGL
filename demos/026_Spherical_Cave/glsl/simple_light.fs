...
vec3 gradient_noise_color(vec3 position)
{
...
vec3 turbulence_color(vec3 position)
{
...
vec3 fractal_sum_color(vec3 position)
{
	const vec3 base_color = vec3(0.7f, 0.9f, 0.15f);
...
vec3 cellular_noise_color(vec3 position)
{
...
vec3 moving_cellular_color(vec3 position)
{
...
vec3 moving_spots_color(vec3 position)
{
...
vec3 star_plasma_color(vec3 position)
{
...
vec3 unknown_color(vec3 position)
{
...
void main()
{
	const vec3 light_color = vec3(1.5f, 1.5f, 0.2f);
	const float light_intensity = 400.0f;

	vec4 e = normalize(view_direction);
    float distance = length(light_direction);
    vec4 l = (light_direction) / distance; 
	vec4 r = reflect(l, normal_ws);


	float dp = dot(normal_ws, l);
	float cos_theta = clamp(dp, 0.0f, 1.0f);
	float cos_alpha = 0.0f;
	if (dp > 0.0f) cos_alpha = clamp(dot(e, r), 0.0f, 1.0f);    

	vec3 color = fractal_sum_color(position_ws.xyz / 40.0);
	vec3 ambient_color = 0.33f * color; 
    vec3 diffuse_color = 0.66f * color;
    vec3 specular_color = normalize(color) * light_color;

    float diffuse_distance_factor = 10.0f / (distance * distance);
    float specular_distance_factor = 15.0f / (distance * distance);

	FragmentColor = vec4(ambient_color +
                    + diffuse_color * light_intensity * light_color * cos_theta * diffuse_distance_factor
                    + specular_color * light_intensity * light_color * pow(cos_alpha, 12) * specular_distance_factor, 1.0f);
