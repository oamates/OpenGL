...
struct tri_stripper
{
    ...
    // Set the minimum size of a triangle strip (should be at least 2 triangles). The stripifier discard any candidate strips that does not satisfy the minimum size condition.
    void SetMinStripSize(size_t MinStripSize = 2);

    // Set the backward search mode in addition to the forward search mode. In forward mode, the candidate strips are build with the current candidate triangle being the first
    // triangle of the strip. When the backward mode is enabled, the stripifier also tests candidate strips where the current candidate triangle is the last triangle of the strip.
    // Enable this if you want better results at the expense of being slightly slower. Note: Do *NOT* use this when the cache optimizer is enabled; it only gives worse results.
    void SetBackwardSearch(bool Enabled = false);
    
    // Set the cache simulator FIFO behavior (does nothing if the cache optimizer is disabled). When enabled, the cache is simulated as a simple FIFO structure. 
    // However, when disabled, indices that trigger cache hits are not pushed into the FIFO structure. This allows simulating some GPUs that do not duplicate cache entries (e.g. NV25 or greater).
...
inline void tri_stripper::SetMinStripSize(const size_t MinStripSize)
    { m_MinStripSize = (MinStripSize < 2) ? 2 : MinStripSize; }
