...
inline void cache_simulator::push(const index i, const bool CountCacheHit)
{
    if (CountCacheHit || m_PushHits)
    {
        ...
        {
            if (CountCacheHit) ++m_NbHits;                                                              // Should we count the cache hits?
            if (!m_PushHits) return;                                                                    // Should we not push the index into the cache if it's a cache hit?
        }
    }
    m_Cache.push_front(i);                                                                              // Manage the indices cache as a FIFO structure
    m_Cache.pop_back();
}
