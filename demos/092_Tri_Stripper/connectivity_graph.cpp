...
namespace{
    ...
    struct tri_edge : public triangle_edge
    {
        tri_edge(index A, index B, size_t TriPos) : triangle_edge(A, B), m_TriPos(TriPos) { }
        size_t TriPos() const { return m_TriPos; }
        size_t  m_TriPos;
    };

    struct cmp_tri_edge_lt
    {
...
void make_connectivity_graph(graph_array<triangle> & Triangles, const indices & Indices)
{
    ...
    for (size_t i = 0; i < Triangles.size(); ++i)                                                           // Fill the triangle data
        Triangles[i] = triangle(Indices[i * 3 + 0], Indices[i * 3 + 1], Indices[i * 3 + 2]);
    
    edge_map EdgeMap;                                                                                       // Build an edge lookup table
    ...
    for (size_t i = 0; i < Triangles.size(); ++i)                                                           // Link neighbour triangles together using the lookup table
    {
        const triangle& Tri = *Triangles[i];
...
{    
    ...
    void LinkNeighbours(graph_array<triangle> & Triangles, const edge_map & EdgeMap, const tri_edge Edge)
    {
        ...
        // See if there are any other edges that are equal (if so, it means that more than 2 triangles are sharing the same edge, which is unlikely but not impossible)
