...
void PrintComputeShaderLimits()
{
	...
	debug_msg("MAX_COMPUTE_WORK_GROUP_INVOCATIONS: %d", workGrpInvocations);
	...
	debug_msg("GL_MAX_COMPUTE_WORK_GROUP_COUNT: X:%d Y:%d Z:%d", X, Y, Z);
	...
	debug_msg("GL_MAX_COMPUTE_WORK_GROUP_SIZE: sizeX:%d sizeY:%d sizeZ:%d", sizeX, sizeY, sizeZ);
}
...
{	
	...
	loadShader("glsl/definitions.glsl", SHADER_HEADER, shaders);
	loadShader("glsl/raygen_cs.glsl", GL_COMPUTE_SHADER, shaders);	
	compileShaderProgram(shaders, raygenprog);	

	shaders.clear();
	shaders.push_back(threadGroupShaderInfo);
	loadShader("glsl/definitions.glsl", SHADER_HEADER, shaders);
	loadShader("glsl/trace.glsl", SHADER_HEADER, shaders);
	loadShader("glsl/rayintersect_cs.glsl", GL_COMPUTE_SHADER, shaders);	
	...
	loadShader("glsl/definitions.glsl", SHADER_HEADER, shaders);
	loadShader("glsl/trace.glsl", SHADER_HEADER, shaders);
	loadShader("glsl/raycol_cs.glsl", GL_COMPUTE_SHADER, shaders);	
...
{
	std::vector<ShaderInfo> shaders;
	loadShader("glsl/simple_vert.glsl", GL_VERTEX_SHADER, shaders);
	loadShader("glsl/simple_frag.glsl", GL_FRAGMENT_SHADER, shaders);
...
{
	return glm::vec2(circleCenterX + radius * glm::cos(angle), circleCenterY + radius * glm::sin(angle));
}
...
int RunRaytracer(int windowWidth, int windowHeight, int threadGroupSize, int renderPasses, int numLights, int numFrames, const char * benchmarkOutputFile = nullptr) {
	...
	std::vector<Light> lightData(MAX_NUM_LIGHTS);
