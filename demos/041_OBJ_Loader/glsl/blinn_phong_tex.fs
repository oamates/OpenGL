...
uniform int flags;                     // logical combination of texture presence flags below
...
const int NORMALMAP_TEXTURE_PRESENT = 0x20;    // the value is read from map_bump sampler, the application should set filtering mode to GL_LINEAR, if both bits are 0, unperturbed normals are used
...
vec3 ambient_color()
{
    if ((flags & AMBIENT_TEXTURE_PRESENT) == 0) 
        return Ka;
...
vec3 diffuse_color()
{
    if ((flags & DIFFUSE_TEXTURE_PRESENT) == 0)
        return Kd;
...
vec3 specular_color()
{
    if ((flags & SPECULAR_TEXTURE_PRESENT) == 0)
        return Ks;
...
{
    if ((flags & SHININESS_TEXTURE_PRESENT) == 0)
        return Ns;
...
vec3 compute_normal()
{
    if ((flags & (HEIGHTMAP_TEXTURE_PRESENT | NORMALMAP_TEXTURE_PRESENT)) == 0) 
        return normalize(normal);

    if ((flags & NORMALMAP_TEXTURE_PRESENT) == 0)       // we got heightmap bump texture
    {
...
void main()
{
    if (((flags & MASK_TEXTURE_PRESENT) != 0) && (d * texture(map_d, uv).r < 0.5f)) discard;
...
//    FragmentColor = vec4(texture(map_Ka, uv).rgb, 1.0f);
//    FragmentColor = vec4(texture(map_Kd, uv).rgb, 1.0f);
//    FragmentColor = vec4(texture(map_Ks, uv).rgb, 1.0f);
