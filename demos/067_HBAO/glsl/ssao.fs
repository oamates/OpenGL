...
void main()
{
    ...
    vec3 col = texture(gdiffuse, TexCoord).rgb;

    //randomization texture
    vec2 fres = vec2(800.0/128.0*5,600.0/128.0*5);
    vec3 random = texture(grandom, TexCoord*fres.xy);
    ...
    float pw = incx;
    float ph = incy;
    float cdepth = texture(gdepth, TexCoord).r;
    ...
    for(float i=0.0; i<3.0; ++i) 
    {
       ...
       gi+=  giFF(ddiff,n,npw,nph)*texture(gdiffuse, TexCoord+vec2(npw,nph)).rgb;
       gi+=  giFF(ddiff2,n,npw,-nph)*texture(gdiffuse, TexCoord+vec2(npw,-nph)).rgb;
       gi+=  giFF(ddiff3,n,-npw,nph)*texture(gdiffuse, TexCoord+vec2(-npw,nph)).rgb;
       gi+=  giFF(ddiff4,n,-npw,-nph)*texture(gdiffuse, TexCoord+vec2(-npw,-nph)).rgb;
       gi+=  giFF(ddiff5,n,0,nph)*texture(gdiffuse, TexCoord+vec2(0,nph)).rgb;
       gi+=  giFF(ddiff6,n,0,-nph)*texture(gdiffuse, TexCoord+vec2(0,-nph)).rgb;
       gi+=  giFF(ddiff7,n,npw,0)*texture(gdiffuse, TexCoord+vec2(npw,0)).rgb;
       gi+=  giFF(ddiff8,n,-npw,0)*texture(gdiffuse, TexCoord+vec2(-npw,0)).rgb;
