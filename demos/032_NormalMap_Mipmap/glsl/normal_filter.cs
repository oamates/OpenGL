...
subroutine(derivative_filter_func) vec2 sobel5x5(sampler2D sampler, vec2 uv, vec2 texel_size, float lod)
{
    ...
    float m2_p2 = texture(sampler, vec2(uvm2.x, uvp2.y), lod).r;
    float m2_p1 = texture(sampler, vec2(uvm2.x, uvp1.y), lod).r;
    float m2_oo = texture(sampler, vec2(uvm2.x, uv00.y), lod).r;
    float m2_m1 = texture(sampler, vec2(uvm2.x, uvm1.y), lod).r;
    float m2_m2 = texture(sampler, vec2(uvm2.x, uvm2.y), lod).r;
    float m1_p2 = texture(sampler, vec2(uvm1.x, uvp2.y), lod).r;
    float m1_p1 = texture(sampler, vec2(uvm1.x, uvp1.y), lod).r;
    float m1_oo = texture(sampler, vec2(uvm1.x, uv00.y), lod).r;
    float m1_m1 = texture(sampler, vec2(uvm1.x, uvm1.y), lod).r;
    float m1_m2 = texture(sampler, vec2(uvm1.x, uvm2.y), lod).r;
    float oo_p2 = texture(sampler, vec2(uv00.x, uvp2.y), lod).r;
    float oo_p1 = texture(sampler, vec2(uv00.x, uvp1.y), lod).r;
    float oo_m1 = texture(sampler, vec2(uv00.x, uvm1.y), lod).r;
    float oo_m2 = texture(sampler, vec2(uv00.x, uvm2.y), lod).r;
    float p1_p2 = texture(sampler, vec2(uvp1.x, uvp2.y), lod).r;
    float p1_p1 = texture(sampler, vec2(uvp1.x, uvp1.y), lod).r;
    float p1_oo = texture(sampler, vec2(uvp1.x, uv00.y), lod).r;
    float p1_m1 = texture(sampler, vec2(uvp1.x, uvm1.y), lod).r;
    float p1_m2 = texture(sampler, vec2(uvp1.x, uvm2.y), lod).r;
    float p2_p2 = texture(sampler, vec2(uvp2.x, uvp2.y), lod).r;
    float p2_p1 = texture(sampler, vec2(uvp2.x, uvp1.y), lod).r;
    float p2_oo = texture(sampler, vec2(uvp2.x, uv00.y), lod).r;
    float p2_m1 = texture(sampler, vec2(uvp2.x, uvm1.y), lod).r;
    float p2_m2 = texture(sampler, vec2(uvp2.x, uvm2.y), lod).r;
