...
template<typename real_t> struct sphere_t
{
    ...
    sphere_t(const glm::tvec3<real_t>& center, real_t radius,
             ...
             real_t reflectivity, real_t transparency)
...
template<typename real_t> glm::tvec3<real_t> trace(const glm::tvec3<real_t>& origin, const glm::tvec3<real_t>& raydir, 
...
{ 
    ...
    glm::tvec3<real_t> phit = origin + raydir * tnear;                                   // point of intersection 
    ...
    { 
        real_t facing_ratio = -glm::dot(raydir, nhit); 
        real_t fresnel_effect = mix(pow(1 - facing_ratio, 3), 1.0f, 0.1f);       // change the mix value to tweak the effect
        ...
        if (sphere->transparency)                                               // if the sphere is also transparent compute refraction ray (transmission)
        {
            ...
            real_t cosi = -glm::dot(nhit, raydir); 
            real_t k = real_t(1.0) - eta * eta * (real_t(1.0) - cosi * cosi); 
            glm::tvec3<real_t> refrdir = raydir * eta + nhit * (eta * cosi - glm::sqrt(k)); 
    ...
    { 
        for (unsigned int i = 0; i < spheres.size(); ++i)                       
        {
            if (spheres[i].emission_color.x > real_t(0.0))
            {
                ...
                glm::tvec3<real_t> albedo_color = sphere->albedo_color;
                color += albedo_color * transmission * std::max(real_t(0.0), glm::dot(nhit, light)) * spheres[i].emission_color; 
            } 
        } 
    } 
...
void ray_trace_thread(const ray_trace_data_t* ray_trace_data, uint8_t* rgb8_image, int y_min, int y_max)
{
    ...
    float inv_res_x = ray_trace_data->inv_res_x;
    float inv_res_y = ray_trace_data->inv_res_y;

    float focal_x = ray_trace_data->focal_x;
    float focal_y = ray_trace_data->focal_y;
    ...
    { 
        ...
        {
            float xx = (2.0f * ((x + 0.5f) * inv_res_x) - 1.0f) * focal_x; 
            float yy = (1.0f - 2.0f * ((y + 0.5f) * inv_res_y)) * focal_y; 
...
{ 
    ...
    ray_trace_data_t ray_trace_data(res_x, res_y, constants::two_pi / 6.0f);

    std::vector<sphere_t>& spheres = ray_trace_data.spheres;
    spheres.push_back(sphere_t(glm::vec3( 0.0f, -10004.0f, -20.0f), 10000.0f, glm::vec3(0.80f, 0.20f, 0.20f), glm::vec3(0.0f), 0.0f, 0.0f));
