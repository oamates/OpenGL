...
struct octree_t
{
    ...
    octree_t(const glm::ivec3& samples, Array3D<float>& sampleData, unsigned int& minLvl, unsigned int& maxLvl, 
             const glm::vec3& offsets, Isosurface* fn, float& complexSurfThresh) 
    ...
    void findGradient(glm::vec3& o_gradient, const glm::ivec3& i_array3dInds);              // finds the gradients at any position in space using the forword difference
