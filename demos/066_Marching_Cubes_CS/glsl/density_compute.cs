...
void main() 
{
	...
	ivec3 index = ivec3(gl_GlobalInvocationID);
     ...
     vec3 vertex_ws = vec3(inverse_view_matrix * vec4(vertex_cs, 1.0f));

	imageStore(density_texture, index, vec4(density_func(vertex_ws), 0.0f, 0.0f, 0.0f));
}
