...
{
    vec4 hash_coord = SGPP_coord_prepare( vec4( gridcell.xy, gridcell.xy + 1.0 ) );                         // gridcell is assumed to be an integer coordinate
...
{                                                                                                    
    gridcell = SGPP_coord_prepare( gridcell );                                                              // gridcell is assumed to be an integer coordinate
...
void SGPP_hash_3D(vec3 gridcell,                                                                            // user definable v1 and v2.  ( 0's and 1's )
                  vec3 v1_mask, vec3 v2_mask,
                  out vec4 hash_0, out vec4 hash_1, out vec4 hash_2)                                        // generates 3 random numbers for each of the 4 3D cell corners.  cell corners:  v0=0,0,0  v3=1,1,1  the other two are user definable
{
...
void SGPP_hash_3D(vec3 gridcell,
                  out vec4 lowz_hash_0, out vec4 lowz_hash_1, out vec4 lowz_hash_2,
                  out vec4 highz_hash_0, out vec4 highz_hash_1, out vec4 highz_hash_2)                      // generates 3 random numbers for each of the 8 cell corners
{    
    gridcell = SGPP_coord_prepare( gridcell );                                                              // gridcell is assumed to be an integer coordinate
    vec3 gridcell_inc1 = step( gridcell, vec3( 287.5 ) ) * ( gridcell + 1.0 );
...
{
    vec4 hash_coord = BBS_coord_prepare(vec4( gridcell.xy, gridcell.xy + 1.0));                             // gridcell is assumed to be an integer coordinate
...
{
    const float DOMAIN = 60.0;                                                                              // gridcell is assumed to be an integer coordinate
    gridcell.xyz = gridcell.xyz - floor(gridcell.xyz * (1.0 / DOMAIN)) * DOMAIN;                            // was having precision issues here with 61.0  60.0 fixes it.  need to test on other cards.
...
// FAST32_hash. A very fast hashing function.  Requires 32bit support.
// The 2D hash formula takes the form :: hash = mod(coord.x * coord.x * coord.y * coord.y, SOMELARGEFLOAT) / SOMELARGEFLOAT
...
{
    const vec2 OFFSET = vec2(26.0, 161.0);                                                                  // gridcell is assumed to be an integer coordinate
...
void FAST32_hash_2D(vec2 gridcell, out vec4 hash_0, out vec4 hash_1, out vec4 hash_2)                       // generates 3 random numbers for each of the 4 cell corners
{    
    const vec2 OFFSET = vec2(26.0, 161.0);                                                                  // gridcell is assumed to be an integer coordinate
    const float DOMAIN = 71.0;
...
vec4 FAST32_hash_3D_Cell(vec3 gridcell)                                                                     // generates 4 different random numbers for the single given cell point
{
    ...
    gridcell.xyz = gridcell - floor(gridcell * (1.0 / DOMAIN)) * DOMAIN;                                    // truncate the domain
...
void FAST32_hash_3D(vec3 gridcell, out vec4 lowz_hash, out vec4 highz_hash)                                 // generates a random number for each of the 8 cell corners
{
    ...
    gridcell.xyz = gridcell.xyz - floor(gridcell.xyz * ( 1.0 / DOMAIN )) * DOMAIN;                          // truncate the domain
    vec3 gridcell_inc1 = step( gridcell, vec3( DOMAIN - 1.5 ) ) * ( gridcell + 1.0 );
    vec4 P = vec4( gridcell.xy, gridcell_inc1.xy ) + OFFSET.xyxy;                                           // calculate the noise
    P *= P;
    P = P.xzxz * P.yyww;
...
void FAST32_hash_3D(vec3 gridcell,                                                                          // user definable v1 and v2.  ( 0's and 1's )
                    vec3 v1_mask, vec3 v2_mask,
                    out vec4 hash_0, out vec4 hash_1, out vec4 hash_2)                                      // generates 3 random numbers for each of the 4 3D cell corners. cell corners: v0=0,0,0 v3=1,1,1 the other two are user definable
{
    const vec2 OFFSET = vec2(50.0, 161.0);
    ...
    gridcell.xyz = gridcell.xyz - floor(gridcell.xyz * (1.0 / DOMAIN)) * DOMAIN;                            // truncate the domain
    vec3 gridcell_inc1 = step(gridcell, vec3(DOMAIN - 1.5)) * (gridcell + 1.0);    
    vec4 P = vec4(gridcell.xy, gridcell_inc1.xy) + OFFSET.xyxy;                                             // compute x*x*y*y for the 4 corners
    P *= P;
    ...
    vec3 lowz_mods  = vec3(1.0 / (SOMELARGEFLOATS.xyz + gridcell.zzz * ZINC.xyz));                          // get the lowz and highz mods
    vec3 highz_mods = vec3(1.0 / (SOMELARGEFLOATS.xyz + gridcell_inc1.zzz * ZINC.xyz));
    v1_mask = (v1_mask.z < 0.5) ? lowz_mods : highz_mods;                                                   // apply mask for v1 and v2 mod values
    v2_mask = (v2_mask.z < 0.5) ? lowz_mods : highz_mods;
    hash_0 = fract(P * vec4(lowz_mods.x, v1_mask.x, v2_mask.x, highz_mods.x));                              // compute the final hash
...
vec4 FAST32_hash_3D(vec3 gridcell,                                                                          // user definable v1 and v2. (0's and 1's)
                    vec3 v1_mask, vec3 v2_mask)                                                             // generates 1 random number for each of the 4 3D cell corners.  cell corners:  v0=0,0,0  v3=1,1,1  the other two are user definable
{
    const vec2 OFFSET = vec2( 50.0, 161.0 );
    const float DOMAIN = 69.0;
    const float SOMELARGEFLOAT = 635.298681;
    ...
    vec4 mod_vals = vec4(1.0 / ( SOMELARGEFLOAT + vec4(gridcell.z, V1z_V2z, gridcell_inc1.z) * ZINC));      // get the z mod vals
    return fract(P * mod_vals);                                                                             // compute the final hash
}
...
void FAST32_hash_3D(vec3 gridcell,
                    out vec4 lowz_hash_0, out vec4 lowz_hash_1, out vec4 lowz_hash_2, out vec4 lowz_hash_3,
                    out vec4 highz_hash_0, out vec4 highz_hash_1, out vec4 highz_hash_2, out vec4 highz_hash_3) //  generates 4 random numbers for each of the 8 cell corners
{
    const vec2 OFFSET = vec2( 50.0, 161.0 );
    const float DOMAIN = 69.0;
    const vec4 SOMELARGEFLOATS = vec4( 635.298681, 682.357502, 668.926525, 588.255119 );
...
// FastHash32_2 :: an alternative to FastHash32
// - slightly slower
...
void FAST32_2_hash_3D(vec3 gridcell,
                      out vec4 z0_hash, out vec4 z1_hash)                                                   // generates a random number for each of the 8 cell corners
{
    ...
    gridcell = gridcell - floor(gridcell * (1.0 / DOMAIN)) * DOMAIN;                                        // truncate the domain
    vec3 gridcell_inc1 = step(gridcell, vec3(DOMAIN - 1.5)) * (gridcell + 1.0);
    gridcell = (gridcell * SCALE) + OFFSET;                                                                 // calculate the noise
...
void FAST32_2_hash_4D(vec4 gridcell,
                      out vec4 z0w0_hash, out vec4 z1w0_hash, out vec4 z0w1_hash, out vec4 z1w1_hash)
{
...
void FAST32_2_hash_4D(vec4 gridcell,
                      out vec4 z0w0_hash_0, out vec4 z0w0_hash_1, out vec4 z0w0_hash_2, out vec4 z0w0_hash_3,
                      out vec4 z1w0_hash_0, out vec4 z1w0_hash_1, out vec4 z1w0_hash_2, out vec4 z1w0_hash_3,
                      out vec4 z0w1_hash_0, out vec4 z0w1_hash_1, out vec4 z0w1_hash_2, out vec4 z0w1_hash_3,
                      out vec4 z1w1_hash_0, out vec4 z1w1_hash_1, out vec4 z1w1_hash_2, out vec4 z1w1_hash_3)
{
    const vec4 OFFSET = vec4( 16.841230, 18.774548, 16.873274, 13.664607 );
    const float DOMAIN = 69.0;
...
//  Interpolation functions (smoothly increase from 0.0 to 1.0 as x increases linearly from 0.0 to 1.0)
float Interpolation_C1(float x) { return x * x * (3.0 - 2.0 * x); }   // 3x^2 - 2x^3
vec2  Interpolation_C1(vec2 x ) { return x * x * (3.0 - 2.0 * x); }   // Hermite Curve
vec3  Interpolation_C1(vec3 x ) { return x * x * (3.0 - 2.0 * x); }
vec4  Interpolation_C1(vec4 x ) { return x * x * (3.0 - 2.0 * x); }

float Interpolation_C2(float x) { return x * x * x * (x * (x * 6.0 - 15.0) + 10.0); }  // 6x^5 - 15x^4 + 10x^3
vec2  Interpolation_C2(vec2 x ) { return x * x * x * (x * (x * 6.0 - 15.0) + 10.0); }  // Quintic Curve
...
float Interpolation_C2_Fast(float x) { float x3 = x * x * x; return (7.0 + (x3 - 7.0) * x) * x3; }  // 7x^3 - 7x^4 + x^7
vec2  Interpolation_C2_Fast(vec2 x ) { vec2  x3 = x * x * x; return (7.0 + (x3 - 7.0) * x) * x3; }  // Faster than Quintic, not as good shape
...
float Interpolation_C3(float x) { float xsq = x*x; float xsqsq = xsq * xsq; return xsqsq * (25.0 - 48.0 * x + xsq * (25.0 - xsqsq)); }  // 25x^4 - 48x^5 + 25x^6 - x^10
vec2  Interpolation_C3(vec2 x ) { vec2  xsq = x*x; vec2  xsqsq = xsq * xsq; return xsqsq * (25.0 - 48.0 * x + xsq * (25.0 - xsqsq)); }  // C3 Interpolation function.
...
// Falloff defined in XSquared (smoothly decrease from 1.0 to 0.0 as xsq increases from 0.0 to 1.0)
...
// Value Noise 2D :: return value range of 0.0->1.0
...
// Value Noise 3D :: return value range of 0.0->1.0
float Value3D(vec3 P)
{    
    vec3 Pi = floor(P);                                                                                     // establish our grid cell and unit position
    vec3 Pf = P - Pi;
...
// Value Noise 4D :: return value range of 0.0->1.0
float Value4D(vec4 P)
{
    vec4 Pi = floor(P);                                                                                     // establish our grid cell and unit position
    vec4 Pf = P - Pi;
    vec4 z0w0_hash;                                                                                         // calculate the hash.
    vec4 z1w0_hash;
    vec4 z0w1_hash;
    vec4 z1w1_hash;
...
// Perlin Noise 2D (gradient noise) :: return value range of -1.0->1.0
float Perlin2D( vec2 P )
{
    ...
    grad_results *= 1.4142135623730950488016887242097;                                                      // (optionally) scale things to a strict -1.0->1.0 range *= sqrt(2)
...
//  Perlin Noise 3D  (gradient noise) :: return value range of -1.0->1.0
...
//  Cellular Noise 3D
//  Speed up by using 2x2x2 search window instead of 3x3x3 produces range of 0.0->1.0
float Cellular3D(vec3 P)
{
    ...
    return min(d1.x, d1.y) * (3.0 / 4.0);    //  scale return value from 0.0->1.333333 to 0.0->1.0   (2/3)^2 * 3  == (12/9) == 1.333333
}
...
// ValueHermite2D :: return value range of -1.0->1.0 (allows for a blend between value and hermite noise)
...
{
    vec2 Pi = floor(P);                                                                                     // establish our grid cell and unit position
    vec2 Pf = P - Pi;
    vec4 hash_value, hash_gradx, hash_grady;
    FAST32_hash_2D( Pi, hash_value, hash_gradx, hash_grady );
    hash_gradx = (hash_gradx - 0.49999) * gradient_scale;                                                  // scale the hash values
...
// ValueHermite3D :: return value range of -1.0->1.0 (allows for a blend between value and hermite noise)
                    ...
                    float normalization_val )   //  normalization_val = ( 1.0 / ( MAXVALUE + MAXGRADIENT * 0.3125 * 3.0 ) )
{
    ...
    hash_gradx0 = (hash_gradx0 - 0.49999) * gradient_scale;                                                // scale the hash values
...
vec3 Value2D_Deriv( vec2 P )
{
    vec2 Pi = floor(P);                                                                                             //  establish our grid cell and unit position
    vec2 Pf = P - Pi;
    vec4 hash = FAST32_hash_2D(Pi);                                                                               //  calculate the hash.
    vec4 blend = Interpolation_C2_InterpAndDeriv(Pf);                                                             //  blend result and return
...
{    
    vec3 Pi = floor(P);                                                                                             //  establish our grid cell and unit position
    vec3 Pf = P - Pi;
    vec4 hash_lowz, hash_highz;                                                                                     // calculate the hash. (various hashing methods listed in order of speed)
    FAST32_hash_3D( Pi, hash_lowz, hash_highz );
    vec3 blend = Interpolation_C2(Pf);                                                                            // blend the results and return
    vec4 res0 = mix(hash_lowz, hash_highz, blend.z);
...
vec3 Perlin2D_Deriv( vec2 P )
{
    vec2 Pi = floor(P);                                                                                             // establish our grid cell and unit position
    vec4 Pf_Pfmin1 = P.xyxy - vec4( Pi, Pi + 1.0 );
    vec4 hash_x, hash_y;                                                                                            // calculate the hash. (various hashing methods listed in order of speed)
    FAST32_hash_2D( Pi, hash_x, hash_y );
    vec4 grad_x = hash_x - 0.49999;                                                                                 // calculate the gradient results
    vec4 grad_y = hash_y - 0.49999;
    vec4 norm = inversesqrt( grad_x * grad_x + grad_y * grad_y );
    grad_x *= norm;
    grad_y *= norm;
    vec4 dotval = ( grad_x * Pf_Pfmin1.xzxz + grad_y * Pf_Pfmin1.yyww );
    vec3 dotval0_grad0 = vec3(dotval.x, grad_x.x, grad_y.x);                                                      //  Convert our data to a more parallel format
    ...
    vec3 k0_gk0 = dotval1_grad1 - dotval0_grad0;                                                                    //  evaluate common constants
    vec3 k1_gk1 = dotval2_grad2 - dotval0_grad0;
    vec3 k2_gk2 = dotval3_grad3 - dotval2_grad2 - k0_gk0;

    
    vec4 blend = Interpolation_C2_InterpAndDeriv(Pf_Pfmin1.xy);                                                   //  C2 Interpolation
...
vec4 Perlin3D_Deriv( vec3 P )
{
    ...
    vec3 blend = Interpolation_C2(Pf);                                                                              // C2 Interpolation
    vec3 blendDeriv = Interpolation_C2_Deriv(Pf);

    float u = blend.x;                                                                                              // calculate final noise + deriv
    float v = blend.y;
    float w = blend.z;
    ...
    result *= 1.1547005383792515290182975610039;                                                                    // (optionally) scale things to a strict -1.0->1.0 range *= 2/sqrt(3)
    return result;
}
...
{    
    ...
    vec4 m = Pf_Pfmin1 * Pf_Pfmin1;                                                                                 // evaluate the surflet
    m = m.xzxz + m.yyww;
    m = max(1.0 - m, 0.0);
    vec4 m2 = m*m;
    vec4 m3 = m*m2;
    ...
    vec4 temp = -6.0 * m2 * grad_results;                                                                           // calc the deriv
...
vec3 Hermite2D_Deriv( vec2 P )
{
    ...
    // NOTE:  This stuff can be optimized further. But it also appears the compiler is doing a lot of that automatically for us anyway
...
vec4 Hermite3D_Deriv( vec3 P )
{
    ...
    vec4 hash_gradx0, hash_grady0, hash_gradz0, hash_gradx1, hash_grady1, hash_gradz1;              // calculate the hash. (various hashing methods listed in order of speed )
    FAST32_hash_3D( Pi, hash_gradx0, hash_grady0, hash_gradz0, hash_gradx1, hash_grady1, hash_gradz1 );
    ...
    float deriv_x = QuinticHermiteDeriv( Pf.x, qh_results_x.x, qh_results_x.y, qh_results_x.z, qh_results_x.w );                    //  for each hermite curve calculate the derivative
    float deriv_y = QuinticHermiteDeriv( Pf.y, qh_results_y.x, qh_results_y.y, qh_results_y.z, qh_results_y.w );
    float deriv_z = QuinticHermiteDeriv( Pf.z, qh_results_z.x, qh_results_z.y, qh_results_z.z, qh_results_z.w );
    
    float finalpos = QuinticHermite( Pf.x, qh_results_x.x, qh_results_x.y, qh_results_x.z, qh_results_x.w );                        // and also the final noise value off any one of them
    return vec4( finalpos, deriv_x, deriv_y, deriv_z ) * FINAL_NORM_VAL;                                                            // normalize and return results! :)
}