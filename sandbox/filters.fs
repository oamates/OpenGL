...
subroutine(filterModeType) vec4 mode_color_hue_filter()
{
   ...
   return vec4(clamp(rgb, 0.0f, 1.0f), 1.0f);
...
subroutine(filterModeType) vec4 mode_perspective_transform_filter()
{
    ...
    else if(gui_perspective_mode == 1)
    {
        ...
        if(D >= 0.0f)
        {
            float x1 = 0.5f * (-bb - sqrt(D)) / aa;
            float x2 = 0.5f * (-bb + sqrt(D)) / aa;
            float u = 0.0f;
            if(x1 >= 0.0f && x1 <= 1.0f)
                u = x1;
            else
            ...
            transf_pos = vec2(u, v);
            color = texture(layerA, transf_pos.xy);
            if(u < 0 || u > 1) color = vec4(0.0f);
            if(v < 0 || v > 1) color = vec4(0.0f);
        }
        else 
            return vec4(0);
    }
...
subroutine(filterModeType) vec4 mode_ao_cancellation_filter()
{
    ...
    return (colorA + (1.0f - colorB) * gui_ao_cancellation) / (1.0f + (1.0f - colorB.a) * gui_ao_cancellation);
}
...
{
	return colorA * (colorA + 2.0f * colorB * (1.0f - colorA));
}
...
{
	return exp(-dot(pos, pos) / (w * w + 1.0f));
}
...
subroutine(filterModeType) vec4 mode_seamless_linear_filter()
{
    ...
    float x   = tc.x;
    float y   = tc.y;
    float rad = 0.5f * make_seamless_radius;
    float pwr = gui_seamless_contrast_power * 2.0f + 0.001f;
    float cst = (gui_seamless_contrast_strenght) * 20.0f;
    // simple seamless - GIMP like behaviour
    if(gui_seamless_mode == 0)
    {
        vec2 offset = -sign(x - 0.5) * vec2(0.5f, 0.0f);
        ...
        float omega =  1.0f - smoothstep(0.0f, rad, tc.x) + smoothstep(1.0f - rad, 1.0f, tc.x);
        float bomega = clamp(omega + omega * pow(length(hA.xyz - hB.xyz) / sqrt(2.0) * cst, pwr), 0.0f, 1.0f);
        return mix(colorA, colorB, bomega);
    }
    else
    {
        vec2 offset = -sign(y - 0.5f) * vec2(0.0f, 0.5f);
        vec4 colorA = texture(layerA, tc);
        vec4 colorB = texture(layerA, vec2(tc.x, tc.y) + offset);//*vec4(1,1,0.4,1);
        vec4 hA = texture(layerB, tc);
        vec4 hB = texture(layerB, vec2(tc.x, tc.y) + offset);
        float omega = 1.0f - smoothstep(0.0f, rad, tc.y) + smoothstep(1.0f - rad, 1.0f, tc.y);
        float bomega = clamp(omega + omega * pow(length(hA.xyz - hB.xyz) / sqrt(2.0f) * cst, pwr), 0.0f, 1.0f);
        return mix(colorA, colorB, bomega);
    }
}
...
subroutine(filterModeType) vec4 mode_seamless_filter()
{
    ...
    if(gui_seamless_mode == 2)
    {
        ...
        if(gui_seamless_mirror_type == 0)
        {
            ...
            if(gui_image_type == 1)
            {
                vec4 nvec = 2.0f * color - 1.0f;
                float sx = sign(x - 0.5f);
                float sy = sign(y - 0.5f);
                color = 0.5f * vec4(sx * nvec.x, sy * nvec.y, nvec.z, nvec.w) + 0.5f;
            }
            return color;
        // X - mirror image
        }
        ...
        {
            vec4 color = texture(layerA, vec2(2.0f * abs(tc.x - 0.5f), tc.y));
            if(gui_image_type == 1)
            {
                vec4 nvec = 2.0f * color - 1.0f;
                float sx = sign(x - 0.5f);
                float sy = sign(y - 0.5f);
                color = 0.5f * vec4(sx * nvec.x, nvec.y, nvec.z, nvec.w) + 0.5f;
            }
            return color;
        // Y - mirror image
        }
        ...
        {
            vec4 color = texture(layerA, vec2(tc.x, 2.0f * abs(tc.y - 0.5f)));
            if(gui_image_type == 1)
            {
                vec4 nvec = 2.0f * color - 1.0f;
                float sx = sign(x - 0.5f);
                float sy = sign(y - 0.5f);
                color = 0.5f * vec4(nvec.x, sy * nvec.y, nvec.z, nvec.w) + 0.5f;
            }
            return color;
        }
    }
...
subroutine(filterModeType) vec4 mode_dgaussians_filter()
{
    ...
    if(gui_specular_amplifier > 0)
        return clamp(3.0f * gui_specular_amplifier * dc, 0.0f, 1.0f);

    return clamp(1.0f + dc * gui_specular_amplifier * 3.0f, 0.0f, 1.0f);
}
...
{
    float c = (1.01f + contrast) / (1.01f - contrast);
    return 0.5f + c * (color - 0.5f);
}
...
subroutine(filterModeType) vec4 mode_constrast_filter()
{
    ...
    icolor = clamp(icolor, 0.0f, 1.0f);
    return clamp(icolor + gui_specular_brightness, 0.0f, 1.0f);
}
...
{
    // if 1.0 the results should be same like without any modyfications
    if(gui_gray_scale_range_tol == 0.0) return 0.0;

    float cdist = length(colorA - colorB);    // distance between two colors
    float tol = gui_gray_scale_range_tol / 5.0;
    return 1 - exp(-tol * cdist); 
    // Example weight calculation it can be any.
...
subroutine(filterModeType) vec4 mode_gray_scale_filter()
{
    ...
    color = vec4(1.0f) * (color.r * gui_gray_scale_preset.r + color.g * gui_gray_scale_preset.g + color.b * gui_gray_scale_preset.b);

    // Additional filtering, for base map conversion enabled based on selected by user min and max values force the colors which are "near"
...
subroutine(filterModeType) vec4 mode_add_noise_filter()
{
    vec4 color = texture(layerA, v2QuadCoords.xy);
...
subroutine(filterModeType) vec4 mode_small_details_filter()
{
    ...
    return clamp(overlay_filter(colorA, colorC * 0.5), 0.0f, 1.0f);
...
subroutine(filterModeType) vec4 mode_medium_details_filter()
{
    ...
    vec4 bluredSmall = gauss_filter(layerA, gui_gauss_w, gui_gauss_radius, 0.5f * gui_depth);
...
subroutine(filterModeType) vec4 mode_normal_mixer_filter()
{
    ...
    vec4 finalColor  = vec4((finalNormal) * 0.5 + 0.5, 1.0f);
    return finalColor;
}
...
subroutine(filterModeType) vec4 mode_sobel_filter()
{
    ...
    vec4 ocolor = vec4(0);
    ocolor.rgb = clamp(0.5 + 0.5 * n, 0.0f, 1.0f);
...
subroutine(filterModeType)  vec4 mode_normal_angle_correction_filter()
{
...
subroutine(filterModeType) vec4 mode_normal_expansion_filter()
{
    ...
    {
        vec3 n = normalize(texture(layerA, v2QuadCoords.xy).xyz * 2.0f - 1.0f);         // get normal
        float slope = (1.0 / (exp(+10 * gui_mix_normals * length(n.xy)) + 1.0f));       // slope based blending param
...
subroutine(filterModeType) vec4 mode_mix_normal_levels_filter()
{
    ...
    vec4 finalColor  = vec4(finalNormal + 0.5f, 1.0f);
    return finalColor;
}
...
float g_scale = 5.0f * gui_ssao_depth;          // dlugosc promienia prubkujacego
float g_bias = 0.05f * gui_ssao_bias;           //  controls the width of the occlusion cone considered by the occludee.
...
subroutine(filterModeType) vec4 mode_occlusion_filter()
{
	...
	ao /= (2 * gui_ssao_no_iters + 1.0) * (2 * gui_ssao_no_iters + 1.0f);

    return vec4(clamp(1 - ao, 0.0f, 1.0f));
}
...
subroutine(filterModeType) vec4 mode_roughness_filter()
{
    ...
    float threshold = gui_roughness_treshold / 20.0;
    ...
    float cc = curvature;
    float dc = clamp(clamp(curvature - abs(threshold), 0.0f, 1.0f) * amplifier, 0.0f, 1.0f);
    if(threshold < 0) 
        dc = 1 - dc;
...
subroutine(filterModeType) vec4 mode_grunge_filter()
{
...
subroutine(filterModeType) vec4 mode_grunge_randomization_filter()
{
...
subroutine(filterModeType) vec4 mode_grunge_normal_warp_filter()
{
...
void main()
{
    ...
    if(material_id >= 0)                        // if current ID is different than -1
    {
        ...
        FragColor = texture(layerA, v2QuadCoords.xy);       // draw last image
    else
        FragColor   = filterMode();                         // normal processing (materials disabled)
}