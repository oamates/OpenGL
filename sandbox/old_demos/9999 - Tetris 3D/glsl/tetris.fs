...
void main()
{
	float r = fract(72185.0f * sin(2351.0f * instanceID + 7134.0f)); 
	float g = fract(12435.0f * sin(3465.0f * instanceID + 9312.0f)); 
	float b = 0.5 * fract(75237.0f * sin(1237.0f * instanceID + 3455.0f)); 

	vec3 rgb = vec3(r, g, b);
	float n = snoise(7.0f * texcoordf + rgb);
	vec3 color = 1.0f * normalize(rgb * rgb) * (vec3(0.5f) + vec3(n * n));	
