...
glsl_program::glsl_program(const glsl_shader& cs)
{
    id = glCreateProgram();
    attach(cs.id); 
    link();         
...
void glsl_program::link()
{
    ...
    if (GL_TRUE == linkage_status) 
    {
        ...
        debug_msg("Program %d successfully linked.");
        return;
    };
    GLint error_msg_length;
    glGetShaderiv (id, GL_INFO_LOG_LENGTH, &error_msg_length);

    debug_msg("glsl_program.link : error_msg_length = %d", error_msg_length);
    if (error_msg_length)
	{

        char * error_msg = (char*) malloc ((size_t) error_msg_length);
        glGetShaderInfoLog (id, error_msg_length, 0, error_msg);
