...
void main(void)
{
    ...
    float strength_factor = 1.0 / (1.0 + 0.00001 * light_distance * light_distance);

    vec3 diffuse_color = flow_noise_color(vec3(texture_coords, 1.0)) * vec3(1.3, 1.3, 0.5);
    vec4 modulator = vec4(diffuse_color * cos_theta, alpha) * strength_factor;
    vec4 additive_component = vec4(diffuse_color, 0.5f) * vec4(0.71, 0.44, 0.44, 1.0f) * vec4(pow(cos_alpha, 16.0)) * strength_factor;
