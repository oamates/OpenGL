...
static const char bmpfont_vssource = "";


bmpfont::bmpfont (const char * file_name) 
{	
	int width, height;
	unsigned char *data = texture::png(name, width, height);
	if (!data)
	{
		debug_msg("Can't open %s font file.", file_name);
		return;
	};
	...
	for(int y = 0; y < 16; y++) 
	{
		for(int x = 0; x < 16; x++, i++) 
		{
			...
			space[index][1] = step - space[index][0] - space[index][1];
		};
	};
	delete [] data;
...
void bmpfont::enable(int w,int h) 
{
	width = w;
	height = h;
...
void bmpfont::puts (float x,float y,const char *str) 
{
...
void bmpfont::printf(float x, float y, const char *format, ...) 
{
	char buf[1024];
	va_list argptr;
...
void bmpfont::printfc(float x, float y, const char *format, ...)
{
	char buf[1024];
	va_list argptr;
	va_start(argptr, format);
