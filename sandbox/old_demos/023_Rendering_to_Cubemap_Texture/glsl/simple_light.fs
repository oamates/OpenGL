...
void main()
{
	...
	for (int i = -3; i <= 3; ++i)
	      	...
	      	for (int k = -3; k <= 3; ++k)
		  	{
			 	float weight = exp(-0.5*(i*i + j*j + k*k));
				material_ambient_color += weight * texture(texture_sampler, vec3(position) + vec3(i, j, k) / 2048.0f);
				total_weight += weight;
			};
    material_ambient_color /= total_weight;
...
//    fragment_color += specular_color;
	fragment_color = material_ambient_color;
