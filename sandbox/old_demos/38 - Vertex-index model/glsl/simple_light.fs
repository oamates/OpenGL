...
const vec4 light_position = vec4(0.0f, 0.0f, 20.0f, 1.0f);
const vec4 light_color = vec4(one_over_root3, one_over_root3, one_over_root3, 1.0f);
const float light_intensity = 300.0f;
...
void main()
{
    ...
    vec4 l = (light_position - position_ws) / distance;      // a direction to a source of light
...
//	vec4 ambient_color = vec4(granite(0.5f * position_ws.xyz), 1.0f) / 7.0f;
//    vec4 material_diffuse_color = mix(ambient_color, vec4(granite(0.5f * normal_ws.xyz), 1.0f) / 7.0f, 0.7f);

	vec4 ambient_color = vec4(marble_color(position_ws.xyz), 1.0f) / 7.0f;
    ...
    float diffuse_distance_factor = 6.0f / (distance * distance);
    float specular_distance_factor = 5.0f / (distance * distance);

	fragment_color = ambient_color +
                     + material_diffuse_color * light_intensity * light_color * lambert_cosine * diffuse_distance_factor
                     + material_specular_color * light_intensity * light_color * pow(cos_alpha, 28) * specular_distance_factor;
