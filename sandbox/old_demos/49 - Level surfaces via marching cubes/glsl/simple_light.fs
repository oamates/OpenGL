...
const float light_intensity = 5000.0f;
...
void main()
{
	...
	float ambient_factor = 0.2 + 0.8 * texture(ssao_image, gl_FragCoord.xy / screen_dim).r;
//	float ambient_factor = texture(ssao_image, gl_FragCoord.xy / screen_dim).r;

	vec4 base_color = vec4(marble_color(position_ws.xyz), 1.0f) / 3.0;         
	
	vec4 ambient_color = ambient_factor * base_color;         

    vec4 material_diffuse_color = mix(base_color, vec4(marble_color(normal_ws.xyz), 1.0f) / 7.0f, 0.2f) * (1.0 + ambient_factor) / 2.0f;

    vec4 material_specular_color = normalize(ambient_color) * light_color;

    float diffuse_distance_factor = 10.0f / (1.0 + 0.5 * distance * distance);
    float specular_distance_factor = 7.0f / (1.0 + 0.5 * distance * distance);
	...
	fragment_color = vec4(vec3(ambient_factor) * unknown_color(vec3(position_ws) / 10.0), 1.0f);
