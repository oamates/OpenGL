...
void main()
{
	...
	vec3 tex_coord0 = 0.1f * (position * 10.0f
	...
	float n0 = 0.5f + 0.5f * snoise(vec3(tex_coord0));

	vec3 color1 = mix(base_color0, base_color1, n0);


	vec3 tex_coord1 = 0.15f * (position + 0.1f * vec3(snoise(position + vec3(0.0, 0.0, time)),
    						           	    snoise(position + vec3(43.0, 17.0, time)),
                                            snoise(position + vec3(-17.0, -43.0, time))));
	float n1      = snoise(tex_coord1 - vec3(0.0, 0.0, time)) + 
		0.50000f * snoise(tex_coord1 *  2.0f - vec3(0.0f, 0.0f, time * 1.4f)) +  
		0.25000f * snoise(tex_coord1 *  4.0f - vec3(0.0f, 0.0f, time * 2.0f)) +
		0.12500f * snoise(tex_coord1 *  8.0f - vec3(0.0f, 0.0f, time * 2.8f)) +
		0.06250f * snoise(tex_coord1 * 16.0f - vec3(0.0f, 0.0f, time * 4.0f)) +
		0.03125f * snoise(tex_coord1 * 32.0f - vec3(0.0f, 0.0f, time * 5.6f)); 

	n1 = n1 * 0.7;

	vec3 color2 = vec3(1.0, 0.5, 0.0) + vec3(n1, n1, n1);

//    FragmentColor = vec4(color1, 1.0f);	
    FragmentColor = vec4(color2, 1.0f);	
//    FragmentColor = vec4(0.5f * (color1 + color2), 1.0f);	
