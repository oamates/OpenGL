...
{
    debug_msg("[vulkan] ObjectType [%i] :: Message :: %s", objectType, pMessage);
    return VK_FALSE;
}
...
static void frame_begin()
{
    ...
    VkRenderPassBeginInfo renderPassBeginInfo = 
    {
        ...
        .framebuffer = g_Framebuffer[g_BackbufferIndices[g_FrameIndex]],
        .renderArea = VkRect2D
        {
            .offset = {0, 0},
    ...
    vkCmdBeginRenderPass(g_CommandBuffer[g_FrameIndex], &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
}
...
int main(int argc, char* argv[])
{
    ...
    if (!glfwInit())
    {
        debug_msg("Can not initialize GLFW");
        return -1;
    }

    glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);
    GLFWwindow* window = glfwCreateWindow(1280, 720, "ImGui Vulkan example", 0, 0);

    // Setup Vulkan
    if (!glfwVulkanSupported())
    {
        debug_msg("GLFW: Vulkan Not Supported\n");
        return -1;
    }
